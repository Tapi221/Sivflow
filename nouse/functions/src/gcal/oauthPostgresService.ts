...
const getKey = (secrets: GoogleOAuthSecrets): Buffer => {
  ...
  try {
    key = Buffer.from(
      requireConfiguredSecret(secrets.tokenEncryptionKey, "GOOGLE_OAUTH_TOKEN_ENCRYPTION_KEY", "token_encryption_key_invalid"),
      "base64",
    );
  } catch (error) {
...
const storeGoogleAccount = async (
...
): Promise<StoredGoogleCalendarAccount> => {
  ...
  const result = await postgresQuery(
    ...
    [uid, profile.accountEmail, profile.accountName, photoUrl, encryptedRefreshToken],
  );
...
const loadGoogleOAuthSecrets = (values: {
  ...
  tokenEncryptionKey: requireConfiguredSecret(
...
const connectGoogleCalendarAccountForUser = async (
  ...
  secrets: GoogleOAuthSecrets,
): Promise<GoogleCalendarAccessResponse> => {
