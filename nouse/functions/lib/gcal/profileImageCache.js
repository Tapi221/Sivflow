...
const isAllowedImageContentType = (contentType) => Boolean(contentType?.startsWith("image/"));
const readImagePayload = async (url) => {
    const response = await fetch(url, { redirect: "follow" });
    const contentType = response.headers.get("content-type")?.split(";")[0]?.trim().toLowerCase() ?? null;
    const contentLength = Number(response.headers.get("content-length") ?? "0");
    if (!response.ok || !isAllowedImageContentType(contentType))
        return null;
    if (Number.isFinite(contentLength) && contentLength > MAX_PROFILE_IMAGE_BYTES)
        return null;
    const payload = Buffer.from(await response.arrayBuffer());
    if (payload.byteLength > MAX_PROFILE_IMAGE_BYTES)
        return null;
    return { contentType, payload };
};
const getStorageProfileImagePath = (uid, sourceUrl) => {
...
const cacheGoogleProfileImageUrl = async (uid, sourceUrl) => {
    if (!sourceUrl)
        return null;
    try {
