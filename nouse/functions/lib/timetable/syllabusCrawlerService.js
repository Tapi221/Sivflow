...
const normalizeText = (value) => value
    ...
    .replace(/\s+/g, " ")
    .trim();
const normalizeSearchText = (values) => values.map((value) => normalizeText(value).toLowerCase()).filter(Boolean).join(" ");
const createHashId = (value) => crypto.createHash("sha256").update(value).digest("hex").slice(0, 32);
...
const clampMaxPages = (value) => Math.max(1, Math.min(MAX_ALLOWED_PAGES, Math.floor(getNumberValue(value, DEFAULT_MAX_PAGES))));
const isPrivateIpv4 = (address) => PRIVATE_IPV4_PATTERNS.some((pattern) => pattern.test(address));
...
const assertFetchableUrl = async (rawUrl) => {
    ...
    if (isPrivateHostName(url.hostname))
        throw new HttpsError("invalid-argument", "Private hostnames cannot be crawled.");
    ...
    if (addresses.some((address) => isBlockedIpAddress(address.address)))
        throw new HttpsError("invalid-argument", "Private network URLs cannot be crawled.");
    url.hash = "";
    return url;
};
const fetchText = async (url) => {
    ...
    if (!response.ok)
        throw new HttpsError("unavailable", `Fetch failed with status ${response.status}.`);
    if (!contentType.includes("text/html") && !contentType.includes("application/xhtml"))
        throw new HttpsError("failed-precondition", "The URL did not return HTML.");
    return { text: (await response.text()).slice(0, MAX_HTML_LENGTH), contentType };
};
const parseRobotsGroups = (robotsText) => {
    const groups = [];
    let currentAgents = [];
    let currentDisallow = [];
    let currentAllow = [];
...
const isRobotsAllowed = async (url) => {
    ...
    try {
        ...
        if (allowedMatches.length === 0 && disallowedMatches.length === 0)
            return true;
...
const parseCourse = (html, url, source) => {
...
const extractCandidateLinks = (html, baseUrl) => {
    ...
    while (match && links.length < MAX_LINKS_PER_PAGE) {
        ...
        try {
            ...
            url.hash = "";
            if (url.origin === baseUrl.origin && COURSE_LINK_PATTERN.test(`${label} ${url.pathname} ${url.search}`))
                links.push(url);
        }
        catch {
            // 不正なリンクは無視する。
...
const upsertTimetableSyllabusSourceRecord = async (input) => {
    const seedUrl = getStringValue(input.seedUrl);
    if (!seedUrl)
        throw new HttpsError("invalid-argument", "seedUrl is required.");
    await assertFetchableUrl(seedUrl);
    const sourceId = getStringValue(input.sourceId) || createHashId(seedUrl);
    await getPostgresPool().query(`insert into timetable_syllabus_sources (
        source_id,
        seed_url,
        institution_name,
        faculty_name,
...
const createCrawlSourceFromInput = (input) => ({
    sourceId: null,
...
const saveCrawlResult = async (jobId, uid, source, courses, scannedPageCount, skippedUrlCount) => {
    const client = await getPostgresPool().connect();
    const savedAt = new Date().toISOString();
    try {
        await client.query("begin");
        await client.query(`insert into timetable_syllabus_crawl_jobs (
          job_id,
          uid,
          source_id,
          seed_url,
        ...
        for (const course of courses) {
            ...
            await client.query(`insert into timetable_syllabus_catalog (
            id,
            source_url,
            source_id,
            institution_name,
          ...
          on conflict (id) do update set
            ...
            await client.query(`insert into timetable_syllabus_crawl_job_courses (
            job_id,
            course_id,
            course_payload,
            created_at,
          ...
          on conflict (job_id, course_id) do update set
            ...
            updated_at = now()`, [jobId, course.id, JSON.stringify(coursePayload)]);
        }
...
const crawlSyllabusSource = async (source, uid) => {
    ...
    while (queue.length > 0 && seen.size < source.maxPages) {
        ...
        try {
            ...
            extractCandidateLinks(text, url).forEach((link) => {
                if (!seen.has(link.toString()) && queue.length + seen.size < source.maxPages)
                    queue.push(link);
            });
        }
        catch {
...
const crawlTimetableSyllabusUrlForUser = async (input, uid) => {
    const source = createCrawlSourceFromInput(input);
    if (!source.seedUrl)
        throw new HttpsError("invalid-argument", "seedUrl is required.");
    return await crawlSyllabusSource(source, uid);
};
const runTimetableSyllabusCatalogCrawlJob = async () => {
    const result = await getPostgresPool().query(`select source_id, seed_url, institution_name, faculty_name, department_name, max_pages
    ...
    for (const row of result.rows) {
        jobs.push(await crawlSyllabusSource({
