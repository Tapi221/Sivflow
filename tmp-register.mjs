...
// packages/backend/server/src/base/config/register.ts
...
// packages/backend/server/src/base/config/env.ts
...
function typeFromShape(shape) {
  switch (shape.constructor) {
    case external_exports.ZodString:
      return "string";
    case external_exports.ZodNumber:
      return "float";
    case external_exports.ZodBoolean:
      return "boolean";
    case external_exports.ZodArray:
      return "array";
    case external_exports.ZodObject:
      return "object";
    case external_exports.ZodOptional:
    case external_exports.ZodNullable:
...
var APP_CONFIG_DESCRIPTORS = {};
var getDescriptors = once_default(() => {
...
function defineModuleConfig(module2, defs) {
  const descriptors = {};
  ...
  APP_CONFIG_DESCRIPTORS[module2] = {
    ...APP_CONFIG_DESCRIPTORS[module2],
    ...descriptors
  };
}
var CONFIG_JSON_PATHS = [
...
function override(config, update) {
  Object.keys(update).forEach((module2) => {
    const moduleDescriptors = APP_CONFIG_DESCRIPTORS[module2];
    if (!moduleDescriptors) {
      return;
    }
    const configKeys = new Set(Object.keys(moduleDescriptors));
    const moduleConfig = config[module2];
    const moduleOverrides = update[module2];
    const merge = (left, right, path = "") => {
      ...
      return mergeWith_default(left, right, (left2, right2, key) => {
        return merge(left2, right2, path === "" ? key : `${path}.${key}`);
      });
    };
    config[module2] = merge(moduleConfig, moduleOverrides);
  });
}
function getDefaultConfig() {
  const config = {};
  const envs = process.env;
  for (const [module2, defs] of Object.entries(APP_CONFIG_DESCRIPTORS)) {
      ...
      if (!success) {
        throw new Error(
          error.issues.map((issue) => {
            return `Invalid config for module [${module2}] with key [${key}]
Value: ${JSON.stringify(defaultValue)}
Error: ${issue.message}`;
          }).join("\n")
        );
      }
      set_default(modulizedConfig, key, defaultValue);
    }
    config[module2] = modulizedConfig;
  }
