...
export type AttachMobileDeviceToHandwritingSessionInput = { session: HandwritingSession;
  mobileDevice: HandwritingDeviceInfo;
  now?: number;
};
export type UpdateHandwritingSessionStatusInput = { session: HandwritingSession;
...
export const isHandwritingSessionActive = (session: Pick<HandwritingSession, "status">): boolean => { return session.status === "waiting" || session.status === "connected";
};
