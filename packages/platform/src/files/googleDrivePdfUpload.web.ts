import type { GoogleDrivePdfUploadInput, GoogleDrivePdfUploadResult } from "./googleDrivePdfUpload.types";
...
export const uploadPdfToGoogleDrive = async ({
  ...
  fileName,
  pdf,
}: GoogleDrivePdfUploadInput): Promise<GoogleDrivePdfUploadResult> => {
  if (!accessToken.trim()) {
