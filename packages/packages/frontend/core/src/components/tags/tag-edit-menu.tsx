...
const DesktopTagEditMenu = ({
      ...
      items: (
        <>
          ...
          <select
            ...
            onChange={e => {
              onTagChange('parentId', e.currentTarget.value);
            }}
          >
            <option value="">
              {t['com.affine.tags.create-tag.no-parent']()}
            </option>
            {parentOptions.map(parent => (
              <option key={parent.id} value={parent.id}>
                {'  '.repeat(parent.depth ?? 0)}
                {parent.name || t['Untitled']()}
              </option>
            ))}
          </select>
          <MenuSeparator />
  ...
  }, [tag, t, jumpToTag, colors, onTagChange, onTagDelete, parentOptions]);
...
const MobileTagEditMenu = ({
  ...
  useEffect(() => {
    if (
      ...
      localTag.parentId !== tag.parentId
    ) {
      setLocalTag({ ...tag });
    }
  }, [tag, localTag.color, localTag.name, localTag.parentId]);
  ...
  const handleOnDone = () => {
    ...
    if ((localTag.parentId ?? '') !== (tag.parentId ?? '')) {
      onTagChange('parentId', localTag.parentId ?? '');
    }
  };
  return (
    <>
      ...
      >
        ...
        <ConfigModal.RowGroup title="Parent">
          <select
            className={styles.parentSelect}
            value={localTag.parentId ?? ''}
            onChange={e => {
              setLocalTag({ ...localTag, parentId: e.currentTarget.value });
            }}
          >
            <option value="">
              {t['com.affine.tags.create-tag.no-parent']()}
