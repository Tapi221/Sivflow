...
export const NavigationPanelOrganize = () => {
  ...
  const handleOnChildrenDrop = useCallback(
    (data: DropTargetDropEvent<AffineDNDData>, node?: FolderNode) => {
      ...
      if (
        ...
        if (entity && isOrganizeSupportType(entity.type)) {
          rootFolder.moveHere(entity.id, rootFolder.indexAt(at, node.id));
  ...
  const createFolderAndDrop = useCallback(
    (data: DropTargetDropEvent<AffineDNDData>) => {
      ...
      if (!isOrganizeSupportType(type) || type === 'folder') {
        return;
      }
      ...
      if (
        ...
        folder.moveHere(data.source.data.from.nodeId, folder.indexAt('after'));
        return;
      }
  ...
  >(
    () => args => {
      const type = args.source.data.entity?.type;
