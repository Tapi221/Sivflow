...
test('should create, get, update, delete auth account', async t => {
  const user = await t.context.user.create({
    email: 'test@affine.pro',
  });
  const authAccount = await t.context.user.createAuthAccount({
    userId: user.id,
    provider: 'test-provider',
    providerAccountId: 'test-provider-account-id',
    accessToken: 'test-access-token',
  });
  t.truthy(authAccount);

  const authAccount2 = await t.context.user.getAuthAccount(
    authAccount.provider,
    authAccount.providerAccountId
  );
  t.truthy(authAccount2);
  t.is(authAccount2!.id, authAccount.id);
  t.is(authAccount2!.user.id, user.id);

  const updatedAuthAccount = await t.context.user.updateAuthAccount(
    authAccount.id,
    {
      accessToken: 'new-access-token',
    }
  );
  t.is(updatedAuthAccount.accessToken, 'new-access-token');
  // get the updated auth account
  const authAccount3 = await t.context.user.getAuthAccount(
    authAccount.provider,
    authAccount.providerAccountId
  );
  t.is(authAccount3!.accessToken, 'new-access-token');

  await t.context.user.deleteAuthAccount(authAccount.id);
  const authAccount4 = await t.context.user.getAuthAccount(
    authAccount.provider,
    authAccount.providerAccountId
  );
  t.is(authAccount4, null);
});
