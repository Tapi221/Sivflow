...
export const TagsEditor = ({
  ...
  const tagOptions: TagOption[] = useMemo(() => {
    ...
    const hierarchicalFilteredTags = flattenedTags.filter(tag =>
      filteredTagIds.has(tag.id)
    );
    if (showCreateTag) {
      return [
        { create: true, value: trimmedInputValue } as const,
        ...hierarchicalFilteredTags,
      ];
    } else {
      return hierarchicalFilteredTags;
    }
  }, [filteredTags, flattenedTags, showCreateTag, trimmedInputValue]);
          ...
          >
            ...
            {tagOptions.map((tag, idx) => {
              ...
              if (isCreateNewTag(tag)) {
                ...
                const parentOptions = flattenedTags.filter(candidate => {
                  if (candidate.id === tag.id) {
                    return false;
                  }
                  return !getDescendantTagIds(tags, tag.id).includes(
                    candidate.id
                  );
                });
                return (
  ...
  const onCreateTag = useCallback(
    (name: string, color: string) => {
      ...
      return {
        ...
        parentId: newTag.parentId$.value,
      };
    },
    [tagService.tagList]
  );
  const onTagChange = useCallback(
    (id: string, property: keyof TagLike, value: string) => {
      if (property === 'name') {
        ...
        tagService.tagList
          .tagByTagId$(id)
          .value?.moveToParent(value || undefined);
      }
    },
    [tagService.tagList]
  );
