...
import { getDescendantTagIds } from '../entities/tree';
...
export class TagList extends Entity {
  ...
  createTag(value: string, color: string, parentId?: string) {
    const newId = this.store.createNewTag(value, color, parentId);
  ...
  tagMetas$ = LiveData.computed(get => {
    return get(this.tags$).map(tag => {
      return {
        ...
        parentId: get(tag.parentId$),
      };
    });
  });

  getDescendantTagIds(tagId: string) {
    return getDescendantTagIds(this.tagMetas$.value, tagId);
  }
