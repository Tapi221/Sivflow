...
import { isOrganizeSupportType } from '../constants';
...
export class FolderNode extends Entity<{
  ...
  info$ = LiveData.from<{
    data: string;
    type: (string & {}) | 'folder' | 'doc' | 'tag' | 'collection' | 'file';
    index: string;
    id: string;
  ...
  filterInvalidChildren(child: { type: string }): boolean {
    if (this.id === null && !isOrganizeSupportType(child.type)) {
      return false;
    }
    return true;
  }
  ...
  createLink(
    ...
    this.store.createLink(this.id, type, data, index);
  }
