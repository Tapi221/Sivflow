...
const useUpdateStatusLabels = (checkUpdateStatus: CheckUpdateStatus) => {
  ...
  const buttonLabel = useMemo(() => {
    if (updateReady) {
      return t['com.affine.aboutSivflow.checkUpdate.button.restart']();
    }
    if (updateAvailable && downloadProgress === null) {
      return t['com.affine.aboutSivflow.checkUpdate.button.download']();
    }
    if (
      checkUpdateStatus === CheckUpdateStatus.LATEST ||
      checkUpdateStatus === CheckUpdateStatus.ERROR
    ) {
      return t['com.affine.aboutSivflow.checkUpdate.button.retry']();
    }
    return t['com.affine.aboutSivflow.checkUpdate.button.check']();
  }, [checkUpdateStatus, downloadProgress, t, updateAvailable, updateReady]);

  const subtitleLabel = useMemo(() => {
    if (updateReady) {
      return t['com.affine.aboutSivflow.checkUpdate.subtitle.restart']();
    } else if (updateAvailable && downloadProgress === null) {
      return t['com.affine.aboutSivflow.checkUpdate.subtitle.update-available']({
        version: updateAvailable.version,
      });
    } else if (checkingForUpdates) {
      return t['com.affine.aboutSivflow.checkUpdate.subtitle.checking']();
    } else if (updateAvailable && downloadProgress !== null) {
      return t['com.affine.aboutSivflow.checkUpdate.subtitle.downloading']();
    } else if (checkUpdateStatus === CheckUpdateStatus.ERROR) {
      return t['com.affine.aboutSivflow.checkUpdate.subtitle.error']();
    } else if (checkUpdateStatus === CheckUpdateStatus.LATEST) {
      return t['com.affine.aboutSivflow.checkUpdate.subtitle.latest']();
    }
    return t['com.affine.aboutSivflow.checkUpdate.subtitle.check']();
  }, [
    checkUpdateStatus,
    downloadProgress,
    checkingForUpdates,
...
export const UpdateCheckSection = () => {
  ...
  return (
    <SettingRow
      name={t['com.affine.aboutSivflow.checkUpdate.title']()}
      desc={subtitle}
    >
      <Button
