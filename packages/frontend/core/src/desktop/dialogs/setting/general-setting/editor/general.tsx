...
const TitleFontSizeSettings = () => {
  ...
  const onTitleFontSizeChange = useCallback(
    (fontSize: number[]) => {
      const size = fontSize[0];
      editorSettingService.editorSetting.set('titleFontSize', size);
      // Update CSS variable immediately
      document.documentElement.style.setProperty(
        '--affine-doc-title-font-size',
        `${size}px`
      );
    },
    [editorSettingService.editorSetting]
  ...
  return (
    <SettingRow
      name={t['com.affine.settings.editorSettings.general.title-font-size.title']()}
      desc={t['com.affine.settings.editorSettings.general.title-font-size.description']()}
    >
      <div className={styles.fontSizeContainer}>
        <Slider
          value={[settings.titleFontSize]}
          onValueChange={onTitleFontSizeChange}
          min={12}
          max={120}
          step={1}
          className={styles.fontSizeSlider}
        />
        <span className={styles.fontSizeValue}>{settings.titleFontSize}px</span>
      </div>
    </SettingRow>
  );
};
