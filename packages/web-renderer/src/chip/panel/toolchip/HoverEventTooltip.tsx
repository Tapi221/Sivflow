...
import { emitHoverTooltipOpen, subscribeHoverTooltipOpen } from "@web-renderer/chip/panel/toolchip/hoverTooltipEvents";
...
const HoverEventTooltip = ({
  ...
  }, [clearCloseTimer, disabled, hasContent, offset, side]);
              ...
              <div className={TOOLTIP_SURFACE_CLASS_NAME}>
                <div className="flex min-w-0 items-start justify-between gap-3">
                  <div className="flex min-w-0 items-start gap-2">
                    <span
                      aria-hidden="true"
                      className="mt-1 h-2 w-2 shrink-0 rounded-full ring-2 ring-white/70"
                      style={{ background: accentColor }}
                    />
                    <span className="min-w-0 flex-1">
                      {tooltipTitle && (
                        <span className="block break-words text-xs font-semibold leading-snug tracking-tight text-slate-600">
                          {tooltipTitle}
                        </span>
                      )}
                      {tooltipSubtitle && (
