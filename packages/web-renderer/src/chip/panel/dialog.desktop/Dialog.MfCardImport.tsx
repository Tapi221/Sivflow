...
import { Dialog, DialogContent, DialogDescription, DialogFooter, DialogHeader, DialogTitle } from "@web-renderer/chip/panel/dialog.desktop/dialog/dialog";
...
const MfCardImportDialog = ({ open, onOpenChange, folderId, folderName, cardSets, onImported, createCardSet, updateCardSet, createCard, ensureTagByName, initialFile = null, initialFileRevision = 0 }: MfCardImportDialogProps) => {
  ...
  const loadFile = useCallback(async (file: File) => {
    ...
    try {
      ...
      setState({ file: loaded.file, loaded });
  ...
  const handleImport = async () => {
    ...
    const destination = destinationMode === "existing"
      ? selectedExistingCardSet ? { kind: "existing-card-set" as const, cardSetId: selectedExistingCardSet.id, cardSetName: selectedExistingCardSet.name } : null
      : newCardSetName.trim() ? { kind: "new-card-set" as const, cardSetName: newCardSetName.trim() } : null;
    if (!destination) {
      toast.error(destinationMode === "existing" ? "追加先のカードセットを選択してください。" : "新規カードセット名を入力してください。");
      return;
    }
    setIsImporting(true);
    try {
      const imported = await importMfCardFile({ cardFile: state.loaded.cardFile, folderId, createCardSet, updateCardSet, createCard, ensureTagByName, destination });
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>{folderName ? `インポート先: ${folderName}` : "選択中のフォルダに単体カードを追加します。"}</DialogDescription>
        </DialogHeader>
        <div className="grid gap-4">
          <div className="grid gap-3 rounded-xl border border-slate-200 bg-slate-50/70 p-4">
            <p className="text-sm font-medium text-slate-800">.mfcard ファイルを選択</p>
          ...
          <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
            ...
            <div className="rounded-xl border border-slate-200 p-3"><p className="text-xs text-slate-500">カード</p><p className="mt-1 text-sm font-medium text-slate-800">{(card?.title?.trim() || card?.questionNumber) ?? "未読み込み"}</p></div>
