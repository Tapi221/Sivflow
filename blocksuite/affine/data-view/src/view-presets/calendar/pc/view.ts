...
const i18n = (key: string, fallback: string) =>
  (I18n as unknown as Record<string, () => string>)[key]?.() || fallback;
...
export class CalendarViewUILogic extends DataViewUILogicBase<CalendarSingleView> {
  ...
  openSetupMenu(target: HTMLElement) {
    const items = this.view.dateProperties$.value.map(property =>
      menu.action({
        name:
          property.name$.value ||
          i18n('com.affine.data-view.calendar-view.date', 'Date'),
        select: () => {
    ...
    if (!this.view.readonly$.value) {
      items.push(
        menu.action({
          name: i18n(
            'com.affine.data-view.calendar-view.create-date-property',
        ...
        source.getSubscriptionOptions?.().map(subscription => {
          const visibilityLabel =
            selectedIds && !selectedIds.has(subscription.id)
              ? i18n('com.affine.data-view.calendar-view.show', 'Show')
              : i18n('com.affine.data-view.calendar-view.hide', 'Hide');

          return menu.action({
            name: `${visibilityLabel} ${subscription.name}`,
            closeOnSelect: false,
            select: () => {
              const allIds = source
    ...
    const connectItems = this.view.externalSources$.value.flatMap(source =>
      source.openConnectSettings
        ? [
            menu.action({
              name: i18n(
                'com.affine.data-view.calendar-view.connect-calendar',
                'Connect calendar'
              ),
              closeOnSelect: false,
              select: () => {
      ...
      ? [
          menu.action({
            name: workspaceCalendar.enabled
              ? i18n(
                  'com.affine.data-view.calendar-view.hide-workspace-calendar',
                  'Hide workspace calendar'
                )
              : i18n(
                  'com.affine.data-view.calendar-view.show-workspace-calendar',
          ...
          menu.action({
            name: i18n(
              'com.affine.data-view.calendar-view.show-all-workspace-calendars',
              'Show all workspace calendars'
            ),
            closeOnSelect: false,
            select: () => {
  ...
  private getDatePropertyMenuItems(
    ...
    if (options?.includeNone) {
      items.push(
        menu.action({
          name: i18n('com.affine.data-view.calendar-view.none', 'None'),
  ...
  getViewOptionsSettingItems(
    ...
    return [
      menu.group({
        name: i18n('com.affine.data-view.calendar-view.date-range', 'Date range'),
        items: [
          menu.action({
            name: i18n(
              'com.affine.data-view.calendar-view.calendar-by',
              'Calendar by'
            ),
            prefix: TodayIcon(),
            closeOnSelect: false,
          ...
          menu.action({
            name: i18n(
              'com.affine.data-view.calendar-view.end-date',
              'End date'
            ),
            prefix: DateTimeIcon(),
            closeOnSelect: false,
            postfix: html`<div
              ...
              >
                ${selectedEnd
                  ...
                  : i18n('com.affine.data-view.calendar-view.none', 'None')}
              </div>
              ${ArrowRightSmallIcon()}`,
            select: () => {
              navigateToSubPage(
                ...
                () =>
                  this.getDatePropertyMenuItems(
                    ...
                    },
                    {
                      includeNone: true,
                      createLabel: i18n(
                        'com.affine.data-view.calendar-view.create-end-date-property',
          ...
          menu.action({
            name: i18n(
              'com.affine.data-view.calendar-view.external-calendars',
              'External calendars'
            ),
    ...
    this.view
      ...
      .catch(() => {
        this.root.config.notification.toast(
          i18n(
            'com.affine.data-view.calendar-view.failed-to-load-calendar-entries',
  ...
  private renderEntry(
    ...
    return html`
      <div
        ...
        i18n('com.affine.data-view.calendar-view.untitled', 'Untitled')}
  ...
  private renderEntryTitle(entry: CalendarEntry): TemplateResult {
    if (entry.kind !== 'row') {
      return html`<span
        ...
        >${entry.title ||
        i18n('com.affine.data-view.calendar-view.untitled', 'Untitled')}</span
      >`;
    }
    ...
    return html`<div class="calendar-empty-month-hint">
      <div class="calendar-empty-month-hint-copy">
        <span class="calendar-empty-month-hint-title"
          >${i18n(
            'com.affine.data-view.calendar-view.empty-hint-title',
            'Nothing here yet'
          )}</span
        >
        <span class="calendar-empty-month-hint-body">
          ${i18n(
            'com.affine.data-view.calendar-view.empty-hint-body',
      ...
      <div class="calendar-empty-month-hint-actions">
        ${this.logic.view.readonly$.value
          ...
          : html`<button
            ...
            >
              ${PlusIcon()}<span
                >${i18n(
                  'com.affine.data-view.calendar-view.new-row',
                  'New row'
                )}</span
              >
            </button>`}
        <button
          ...
          aria-label=${i18n('com.affine.data-view.calendar-view.dismiss', 'Dismiss')}
        ...
        <div class="calendar-toolbar">
          ...
          <div class="calendar-nav">
            ${this.logic.isCurrentMonth()
              ...
              : html`<button
                ...
                >
                  <span
                    >${i18n(
                      'com.affine.data-view.calendar-view.today',
                      'Today'
                    )}</span
                  >
                </button>`}
            <button
              ...
              aria-label=${i18n(
                'com.affine.data-view.calendar-view.previous-month',
                'Previous month'
              )}
            ...
            <button
              class="calendar-icon-button"
              aria-label=${i18n(
                'com.affine.data-view.calendar-view.next-month',
                'Next month'
              )}
              @click=${() => this.logic.moveMonth(1)}
            >
        ...
        ${setup
          ? html`<div class="calendar-setup">
              ...
              >
                ${TodayIcon()}<span
                  >${i18n(
                    'com.affine.data-view.calendar-view.select-or-create-date-property',
                    'Select or create date property'
                  )}</span
                >
              </button>
            </div>`
