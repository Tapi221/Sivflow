...
export class DatabaseBlockComponent extends CaptionedBlockComponent<DatabaseBlockModel> {
  private readonly clickDatabaseOps = (e: MouseEvent) => {
    const i18n = this.std.getOptional(I18nStringProvider);
    const t = (key: string, fallback: string) => i18n?.t(key) ?? fallback;
    const options = this.optionsConfig.configure(this.model, {
      items: [
        menu.input({
          initialValue: this.model.props.title.toString(),
          placeholder: t('com.affine.database.title.placeholder', 'Database title'),
          onChange: text => {
        ...
        menu.action({
          prefix: CommentIcon(),
          name: t('com.affine.database.ops.comment', 'Comment'),
        ...
        menu.action({
          prefix: CopyIcon(),
          name: t('com.affine.database.ops.copy', 'Copy'),
          select: () => {
            const slice = Slice.fromModels(this.store, [this.model]);
            this.std.clipboard
              .copySlice(slice)
              .then(() => {
                toast(this.host, t('com.affine.database.ops.copied', 'Copied to clipboard'));
              })
              .catch(console.error);
          },
        }),
        menu.group({
          items: [
            menu.action({
              ...
              name: t('com.affine.database.ops.delete', 'Delete Database'),
              select: () => {
