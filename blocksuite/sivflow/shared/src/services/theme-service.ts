...
export class ThemeService extends Extension {
   ...
   * `#fff`
   * `light-dark(#fff, #000)`
   * `var(--sivflow-palette-shape-blue)`
   * ```
   */
  ...
  getCssVariableColor(property: string, theme = this.theme) {
    ...
    if (property.startsWith('--')) {
      ...
      const affineProperty = toAffineCssVariable(property);
      const key = affineProperty as keyof AffineCssVariables;
      // V1 theme
      const variables =
        theme === ColorScheme.Dark
          ...
          : combinedLightCssVariables;
      const color =
        variables[key] ?? variables[property as keyof AffineCssVariables];
      ...
      return (
        ...
        rootStyle.getPropertyValue(affineProperty).trim() ||
        DefaultTheme.transparent
      );
    }
    return property;
