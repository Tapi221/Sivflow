  ...
  _addViewMenu = (event: MouseEvent) => {
    popFilterableSimpleMenu(
      ...
      this.dataSource.viewMetas.map(v => {
        return menu.action({
          name: typeof v.model.defaultName === 'function' ? v.model.defaultName() : v.model.defaultName,
      ...
      [
        ...
        menu.group({
          items: this.dataSource.viewMetas.map(v => {
            return menu.action({
              name: `Create ${typeof v.model.defaultName === 'function' ? v.model.defaultName() : v.model.defaultName}`,
              hide: () => this.readonly,
              prefix: PlusIcon(),
              select: () => {
                this.addView(v.type);
