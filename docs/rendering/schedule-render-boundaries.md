...
- `SidebarLayeredDirectory` はアプリ共通の左サイドバー外枠とナビゲーションを持つ
- `CalendarSidebarContent` は Schedule 用のカレンダー・プロジェクト一覧を持つ
- `CalendarSidebarController` は Google Calendar / project calendar の状態を接続する

`activeMode` に反応してよいのは、Schedule 用リスト表示のうち mode に依存する領域だけ。
...
6. 左端 `Sidebar` に Schedule 内部の state を渡していないか
7. Schedule 内の mode 切替だけで URL を変更していないか
8. 白いパネルの見た目変更を content 側の状態変更と混ぜていないか
9. `CarvePanelChrome` に `children` や mode 固有データを渡していないか
 ...
 * - 更新してよい条件は対象コンポーネントが直接表示する state の変更だけ。
 * - activeMode / selectedRange / googleAccounts など不要な props を追加しない。
 */
```
