...
**概要:** Phase2で予定しているアーキテクチャ改良（スケーラビリティ、同期基盤の改善、運用性向上）を整理した仕様書です。

**重要ポイント:**
- 差分同期のページングとプッシュバッチ分割によりFirestoreの制限を回避します。
- コンポーネント分割（QueueManager, DiffEngine, CloudAdapter）でテスト容易性を高めます。
- モニタリングとリトライ設計を強化して運用耐性を向上させます。
