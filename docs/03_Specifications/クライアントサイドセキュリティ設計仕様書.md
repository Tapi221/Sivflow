...
**概要:** クライアント側でのセキュリティ監視・UI制御・同期停止ロジックを定義した設計書です。

**重要ポイント:**
- `SecurityMonitor` が Firestore を監視し、グローバルステートで警告やロックを反映します。
- アカウントロック時はSyncを停止し、UIで全画面ロックまたは警告バナーを表示します。
- 運用用ツール（検証スクリプト、コンソール操作）とルール連携により即時制御を実現します。
