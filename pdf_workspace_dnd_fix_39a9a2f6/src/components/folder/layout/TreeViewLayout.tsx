...
const TreeViewLayout = ({
  ...
  const isSidebarContentCollapsed =
    selectedItem?.type === "document" && selectedDocument?.kind === "pdf";

  const handlePdfDocumentUpdate = useCallback(
  ...
  const paneLayout = (
    <>
      <TreeViewSidebar
        ...
        collapsedContent={
          selectedDocument?.kind === "pdf" ? <PdfThumbnailSidebar /> : null
        }
      >
        {sidebarContent}
      </TreeViewSidebar>
  ...
  const mainContent: ReactNode =
    selectedDocument?.kind === "pdf" ? (
      <PdfWorkspaceProvider
        ...
        onDocumentUpdate={handlePdfDocumentUpdate}
      >
        {paneLayout}
      </PdfWorkspaceProvider>
    ) : (
