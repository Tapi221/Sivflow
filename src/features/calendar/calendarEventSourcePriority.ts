...
const createCalendarYearEventDisplayResolver = ({ appProjects, projectCalendarLinks, googleAccounts }: CalendarEventSourcePriorityInput): CalendarYearEventDisplayResolver => { const projectById = new Map(appProjects.map((project) => [project.id, project]));
