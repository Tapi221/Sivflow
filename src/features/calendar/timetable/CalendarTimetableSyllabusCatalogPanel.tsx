    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/20 px-4 py-6" role="dialog" aria-modal="true" aria-label="授業DB">
      <div className="flex max-h-full w-full max-w-[980px] flex-col overflow-hidden rounded-[22px] border border-[#e5e5ea] bg-white shadow-[0_18px_60px_rgba(0,0,0,0.18)]">
        <div className="flex shrink-0 items-center justify-between border-b border-[#f0f0f2] px-5 py-4">
          <div><h2 className="text-[17px] font-bold text-[#1c1c1e]">授業DB</h2><p className="mt-1 text-[12px] font-medium text-[#8e8e93]">大学・学部・授業を登録して時間割に追加</p></div>
        ...
        <div className="grid min-h-0 flex-1 grid-cols-1 overflow-hidden lg:grid-cols-[360px_minmax(0,1fr)]">
          <div className="min-h-0 overflow-y-auto border-b border-[#f0f0f2] px-5 py-4 lg:border-b-0 lg:border-r">
            <div className="mb-4 flex items-center justify-between"><div className="text-[13px] font-bold text-[#1c1c1e]">授業を登録</div><div className="rounded-full bg-[#f7f7f8] px-2 py-1 text-[11px] font-bold text-[#8e8e93]">{catalogCountLabel}</div></div>
            {institutions.length > 0 ? <div className="mb-3 flex flex-wrap gap-2">{institutions.slice(0, 8).map((institution) => <button key={institution.id} type="button" className="rounded-full border border-[#e5e5ea] px-2.5 py-1 text-[11px] font-bold text-[#6e6e73]" onClick={() => setInstitutionName(institution.name)}>{institution.name}</button>)}</div> : null}
            <div className="grid gap-2">
              <input className="h-10 rounded-[12px] border px-3 text-[13px] font-semibold" value={institutionName} onChange={(event) => setInstitutionName(event.target.value)} placeholder="大学・専門学校名" />
              <select className="h-10 rounded-[12px] border px-3 text-[13px] font-semibold" value={institutionKind} onChange={(event) => setInstitutionKind(event.target.value as CalendarTimetableInstitutionKind)} aria-label="学校種別"><option value="university">大学</option><option value="vocational">専門学校</option><option value="college">短大</option><option value="other">その他</option></select>
              <input className="h-10 rounded-[12px] border px-3 text-[13px] font-semibold" value={facultyName} onChange={(event) => setFacultyName(event.target.value)} placeholder="学部" />
              <input className="h-10 rounded-[12px] border px-3 text-[13px] font-semibold" value={departmentName} onChange={(event) => setDepartmentName(event.target.value)} placeholder="学科・専攻" />
              <input className="h-10 rounded-[12px] border px-3 text-[13px] font-semibold" value={title} onChange={(event) => setTitle(event.target.value)} placeholder="授業名" />
              <div className="grid grid-cols-2 gap-2"><input className="h-10 rounded-[12px] border px-3 text-[13px] font-semibold" value={room} onChange={(event) => setRoom(event.target.value)} placeholder="教室" /><input className="h-10 rounded-[12px] border px-3 text-[13px] font-semibold" value={teacher} onChange={(event) => setTeacher(event.target.value)} placeholder="担当教員" /></div>
              <div className="grid grid-cols-2 gap-2"><input className="h-10 rounded-[12px] border px-3 text-[13px] font-semibold" value={semesterLabel} onChange={(event) => setSemesterLabel(event.target.value)} placeholder="前期 / 春学期" /><input className="h-10 rounded-[12px] border px-3 text-[13px] font-semibold" value={credits} onChange={(event) => setCredits(event.target.value)} placeholder="単位数" /></div>
              <input className="h-10 rounded-[12px] border px-3 text-[13px] font-semibold" value={syllabusUrl} onChange={(event) => setSyllabusUrl(event.target.value)} placeholder="シラバスURL" />
              <textarea className="min-h-[64px] rounded-[12px] border px-3 py-2 text-[13px] font-medium" value={memo} onChange={(event) => setMemo(event.target.value)} placeholder="メモ" />
              <div><div className="mb-2 text-[11px] font-bold text-[#6e6e73]">色</div><div className="flex flex-wrap gap-1.5">{TAG_COLOR_KEYS.map((key) => <button key={key} type="button" className={cn("h-7 w-7 rounded-full border-2", colorKey === key ? "border-[#007aff]" : "border-transparent")} style={getTagColorStyle(key)} onClick={() => setColorKey(key)} aria-label={`色 ${key}`} />)}</div></div>
              <div><div className="mb-2 flex items-center justify-between"><span className="text-[11px] font-bold text-[#6e6e73]">曜日・時限</span><button type="button" className="rounded-full px-2 py-1 text-[11px] font-bold text-[#007aff] disabled:text-[#c7c7cc]" disabled={periods.length === 0} onClick={handleAddSlot}>追加</button></div><div className="grid gap-2">{slots.map((slot, index) => <div key={index} className="grid grid-cols-[minmax(0,1fr)_minmax(0,1fr)_auto] gap-2"><select className="h-9 rounded-[10px] border px-2 text-[12px] font-bold" value={slot.dayIndex} onChange={(event) => { const nextDayIndex = Number(event.target.value); if (isValidCalendarTimetableWeekdayIndex(nextDayIndex)) handleUpdateSlot(index, { ...slot, dayIndex: nextDayIndex }); }} aria-label="曜日">{TIMETABLE_DAY_LABELS.map((label, dayIndex) => <option key={label} value={dayIndex}>{label}</option>)}</select><select className="h-9 rounded-[10px] border px-2 text-[12px] font-bold" value={slot.periodLabel} onChange={(event) => handleUpdateSlot(index, { ...slot, periodLabel: event.target.value })} aria-label="時限">{periods.map((period) => <option key={period.id} value={period.label}>{period.label}限</option>)}</select><button type="button" className="rounded-full px-2 text-[12px] font-bold text-[#ff3b30]" onClick={() => handleDeleteSlot(index)}>削除</button></div>)}</div></div>
