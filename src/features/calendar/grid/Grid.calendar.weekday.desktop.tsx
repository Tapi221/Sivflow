...
const CalendarWeekDayGridComponent = ({ headerScrollRef, allDayScrollRef, scrollContainerRef, visibleDays, visibleEvents, calendarGridStyle, allDayEventOrder, onScroll, selectedDate, onSelectDate, onMoveCalendarEvent, onReorderAllDayEvents }: CalendarWeekDayGridProps) => {
  ...
  const setDragStateValue = useCallback((nextDragState: WeekdayEventDragState | null) => {
    dragStateRef.current = nextDragState; setDragState(nextDragState); }, []);
  const setAllDayColumnRef = useCallback((dayKey: string) => (element: HTMLDivElement | null) => {
    if (element) {
      allDayColumnRefs.current.set(dayKey, element); return; } allDayColumnRefs.current.delete(dayKey); }, []);
  const setDayColumnRef = useCallback((dayKey: string) => (element: HTMLDivElement | null) => {
    if (element) {
      dayColumnRefs.current.set(dayKey, element); return; } dayColumnRefs.current.delete(dayKey); }, []);
  const getDayColumnAtClientX = useCallback((clientX: number): WeekdayDayColumnHit | null => {
    let nearestHit: WeekdayDayColumnHit | null = null; let nearestDistance = Number.POSITIVE_INFINITY; for (const day of visibleDays) {
      const key = getCalendarDateKey(day); const element = dayColumnRefs.current.get(key); if (!element) continue; const rect = element.getBoundingClientRect(); if (clientX >= rect.left && clientX <= rect.right) return { day, element }; const distance = clientX < rect.left ? rect.left - clientX : clientX - rect.right; if (distance < nearestDistance) {
        nearestDistance = distance; nearestHit = { day, element }; } } return nearestHit; }, [visibleDays]);
  const getAllDayColumnAtClientX = useCallback((clientX: number): WeekdayDayColumnHit | null => {
    let nearestHit: WeekdayDayColumnHit | null = null; let nearestDistance = Number.POSITIVE_INFINITY; for (const day of visibleDays) {
      const key = getCalendarDateKey(day); const element = allDayColumnRefs.current.get(key); if (!element) continue; const rect = element.getBoundingClientRect(); if (clientX >= rect.left && clientX <= rect.right) return { day, element }; const distance = clientX < rect.left ? rect.left - clientX : clientX - rect.right; if (distance < nearestDistance) {
        nearestDistance = distance; nearestHit = { day, element }; } } return nearestHit; }, [visibleDays]);
  const getAllDayDragPreview = useCallback((clientX: number, clientY: number): WeekdayEventDragPreview | null => {
    const allDayElement = allDayScrollRef?.current; if (!allDayElement) return null; const rect = allDayElement.getBoundingClientRect(); if (clientY < rect.top || clientY > rect.bottom) return null; const hit = getAllDayColumnAtClientX(clientX); if (!hit) return null; const previewStartsAt = startOfDay(hit.day); const previewEndsAt = addDays(previewStartsAt, 1); const previewAllDayIndex = getAllDayDropIndex(hit.element, clientY); return { previewStartsAt, previewEndsAt, previewIsAllDay: true, previewAllDayIndex, previewColumnDayKey: null }; }, [allDayScrollRef, getAllDayColumnAtClientX]);
