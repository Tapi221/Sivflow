...
const getCurrentTimeTopStyle = (now: Date): CSSProperties => ({ top: `calc(${(now.getHours() * GRID.WEEKDAY_MINUTES_PER_HOUR + now.getMinutes()) / GRID.WEEKDAY_MINUTES_PER_HOUR} * var(${GRID.WEEKDAY_CSS_VAR_HOUR_ROW_HEIGHT}))` });
...
const getTimedEntryMaxMinHeightHours = (entry: CalendarTimeGridLayoutEntry, entries: readonly CalendarTimeGridLayoutEntry[], rangeHours: number): number | undefined => {
  ...
  const nextEntry = entries.filter((candidate) => candidate !== entry && doEntriesOverlapHorizontally(entry, candidate)).map((candidate) => getWeekdayTimedEventFrame(candidate, rangeHours)).filter((candidateFrame) => candidateFrame.topHours > frame.topHours).sort((a, b) => a.topHours - b.topHours)[0];
...
const getDragPreviewStyle = (state: WeekdayEventDragState, day: Date): CSSProperties => {
  ...
  return { left: `${leftPx}px`, top: `calc(${startMinutes / GRID.WEEKDAY_MINUTES_PER_HOUR} * var(${GRID.WEEKDAY_CSS_VAR_HOUR_ROW_HEIGHT}))`, width: `calc(100% - ${widthInsetPx}px)`, height: `calc(${visibleDurationMinutes / GRID.WEEKDAY_MINUTES_PER_HOUR} * var(${GRID.WEEKDAY_CSS_VAR_HOUR_ROW_HEIGHT}) - ${eventChipDesign.weekdayGrid.timedVerticalTrimPx}px)`, minHeight: `${eventChipDesign.weekdayGrid.timedMinHeightPx}px`, pointerEvents: "none", ...CALENDAR_EVENT_DRAGGING_STYLE };
};
...
const CalendarWeekDayGridComponent = ({ headerScrollRef, allDayScrollRef, scrollContainerRef, visibleDays, visibleEvents, calendarGridStyle, allDayEventOrder, onScroll, selectedDate, onSelectDate, onMoveCalendarEvent, onReorderAllDayEvents }: CalendarWeekDayGridProps) => {
  ...
  const setDragStateValue = useCallback((nextDragState: WeekdayEventDragState | null) => { dragStateRef.current = nextDragState; setDragState(nextDragState); }, []);
  const setAllDayColumnRef = useCallback((dayKey: string) => (element: HTMLDivElement | null) => { if (element) { allDayColumnRefs.current.set(dayKey, element); return; } allDayColumnRefs.current.delete(dayKey); }, []);
  const setDayColumnRef = useCallback((dayKey: string) => (element: HTMLDivElement | null) => { if (element) { dayColumnRefs.current.set(dayKey, element); return; } dayColumnRefs.current.delete(dayKey); }, []);
  const getDayColumnAtClientX = useCallback((clientX: number): WeekdayDayColumnHit | null => { let nearestHit: WeekdayDayColumnHit | null = null; let nearestDistance = Number.POSITIVE_INFINITY; for (const day of visibleDays) { const key = getCalendarDateKey(day); const element = dayColumnRefs.current.get(key); if (!element) continue; const rect = element.getBoundingClientRect(); if (clientX >= rect.left && clientX <= rect.right) return { day, element }; const distance = clientX < rect.left ? rect.left - clientX : clientX - rect.right; if (distance < nearestDistance) { nearestDistance = distance; nearestHit = { day, element }; } } return nearestHit; }, [visibleDays]);
  const getAllDayColumnAtClientX = useCallback((clientX: number): WeekdayDayColumnHit | null => { let nearestHit: WeekdayDayColumnHit | null = null; let nearestDistance = Number.POSITIVE_INFINITY; for (const day of visibleDays) { const key = getCalendarDateKey(day); const element = allDayColumnRefs.current.get(key); if (!element) continue; const rect = element.getBoundingClientRect(); if (clientX >= rect.left && clientX <= rect.right) return { day, element }; const distance = clientX < rect.left ? rect.left - clientX : clientX - rect.right; if (distance < nearestDistance) { nearestDistance = distance; nearestHit = { day, element }; } } return nearestHit; }, [visibleDays]);
  const getAllDayDragPreview = useCallback((clientX: number, clientY: number): WeekdayEventDragPreview | null => { const allDayElement = allDayScrollRef?.current; if (!allDayElement) return null; const rect = allDayElement.getBoundingClientRect(); if (clientY < rect.top || clientY > rect.bottom) return null; const hit = getAllDayColumnAtClientX(clientX); if (!hit) return null; const previewStartsAt = startOfDay(hit.day); const previewEndsAt = addDays(previewStartsAt, 1); const previewAllDayIndex = getAllDayDropIndex(hit.element, clientY); return { previewStartsAt, previewEndsAt, previewIsAllDay: true, previewAllDayIndex, previewColumnDayKey: null }; }, [allDayScrollRef, getAllDayColumnAtClientX]);
  const getTimedDragPreview = useCallback((state: WeekdayEventDragState, clientX: number, clientY: number): WeekdayEventDragPreview | null => { const hit = getDayColumnAtClientX(clientX); if (!hit) return null; const rect = hit.element.getBoundingClientRect(); const rowHeight = getHourRowHeightPx(hit.element); const rawStartMinutes = ((clientY - rect.top) / rowHeight) * GRID.WEEKDAY_MINUTES_PER_HOUR - state.pointerOffsetMinutes; const maxStartMinutes = Math.max(0, WEEKDAY_TIMELINE_RANGE_MINUTES - WEEKDAY_TIMED_EVENT_DRAG_SNAP_MINUTES); const startMinutes = clampMinutes(snapMinutes(rawStartMinutes), maxStartMinutes); const previewStartsAt = addMinutes(startOfDay(hit.day), startMinutes); const previewEndsAt = new Date(previewStartsAt.getTime() + state.durationMs); const previewColumnDayKey = getCalendarDateKey(hit.day); return { previewStartsAt, previewEndsAt, previewIsAllDay: false, previewAllDayIndex: null, previewColumnDayKey }; }, [getDayColumnAtClientX]);
  ...
  const updateDragPreviewFromSnapshot = useCallback((snapshot: CalendarEventDragPointerSnapshot) => { const state = dragStateRef.current; if (!state || snapshot.pointerId !== state.pointerId) return; const preview = getDragPreviewTimes(state, snapshot.clientX, snapshot.clientY); if (!preview) return; if (state.previewIsAllDay === preview.previewIsAllDay && state.previewAllDayIndex === preview.previewAllDayIndex && state.previewColumnDayKey === preview.previewColumnDayKey && areSameCalendarEventTimes(state.previewStartsAt, state.previewEndsAt, preview.previewStartsAt, preview.previewEndsAt)) return; setDragStateValue({ ...state, ...preview }); }, [getDragPreviewTimes, setDragStateValue]);
  ...
  const updateDragPreview = useCallback((event: PointerEvent) => { const state = dragStateRef.current; if (!state || event.pointerId !== state.pointerId) return; const snapshot = createCalendarEventDragPointerSnapshot(event.pointerId, event.buttons, event.clientX, event.clientY); updateDragAutoScroll(snapshot, !state.previewIsAllDay); updateDragPreviewFromSnapshot(snapshot); event.preventDefault(); }, [updateDragAutoScroll, updateDragPreviewFromSnapshot]);
  const commitAllDayOrder = useCallback((state: WeekdayEventDragState) => { if (!state.previewIsAllDay || state.previewAllDayIndex === null) return; const targetDayKey = getCalendarDateKey(state.previewStartsAt); const targetEvents = getOrderedAllDayEvents(allDayEvents.get(targetDayKey) ?? [], targetDayKey, allDayEventOrder); const currentEventKeys = targetEvents.map(createCalendarEventKey); const orderedEventKeys = insertEventKeyAtIndex(currentEventKeys, state.eventKey, state.previewAllDayIndex); if (targetDayKey === state.sourceDayKey && areSameEventKeyOrder(currentEventKeys, orderedEventKeys)) return; onReorderAllDayEvents?.({ eventKey: state.eventKey, sourceDayKey: state.sourceDayKey, targetDayKey, orderedEventKeys }); }, [allDayEventOrder, allDayEvents, onReorderAllDayEvents]);
  const commitDragState = useCallback((state: WeekdayEventDragState) => { commitAllDayOrder(state); if (isSameCalendarEventMove(state.event, state.previewStartsAt, state.previewEndsAt, state.previewIsAllDay)) return; void Promise.resolve(onMoveCalendarEvent?.({ event: state.event, startsAt: state.previewStartsAt, endsAt: state.previewEndsAt, isAllDay: state.previewIsAllDay })).catch((error: unknown) => { console.warn("[CalendarWeekDayGrid] calendar event move failed", error); }); }, [commitAllDayOrder, onMoveCalendarEvent]);
  const finishDrag = useCallback((event: PointerEvent, shouldCommit: boolean) => { const state = dragStateRef.current; if (!state || event.pointerId !== state.pointerId) return; event.preventDefault(); const dragElement = dragElementRef.current; if (dragElement?.hasPointerCapture(event.pointerId)) dragElement.releasePointerCapture(event.pointerId); dragElementRef.current = null; stopDragAutoScroll(); setDragStateValue(null); if (shouldCommit) commitDragState(state); }, [commitDragState, setDragStateValue, stopDragAutoScroll]);
  const startEventDrag = useCallback((event: ReactPointerEvent<HTMLDivElement>, calendarEvent: GoogleCalendarEvent, pointerOffsetMinutes: number, durationMs: number, sourceColumnDayKey: string | null) => { if (event.button !== 0 || !isCalendarEventDraggable(calendarEvent, onMoveCalendarEvent)) return; const eventKey = createCalendarEventKey(calendarEvent); const sourceDayKey = getCalendarDateKey(calendarEvent.startsAt); const preview = getDragPreviewTimes({ eventKey, event: calendarEvent, pointerId: event.pointerId, pointerOffsetMinutes, durationMs, sourceDayKey, previewStartsAt: calendarEvent.startsAt, previewEndsAt: calendarEvent.endsAt, previewIsAllDay: calendarEvent.isAllDay, previewAllDayIndex: null, previewColumnDayKey: sourceColumnDayKey }, event.clientX, event.clientY); event.preventDefault(); event.stopPropagation(); event.currentTarget.setPointerCapture(event.pointerId); dragElementRef.current = event.currentTarget; setDragStateValue({ eventKey, event: calendarEvent, pointerId: event.pointerId, pointerOffsetMinutes, durationMs, sourceDayKey, previewStartsAt: preview?.previewStartsAt ?? calendarEvent.startsAt, previewEndsAt: preview?.previewEndsAt ?? calendarEvent.endsAt, previewIsAllDay: preview?.previewIsAllDay ?? calendarEvent.isAllDay, previewAllDayIndex: preview?.previewAllDayIndex ?? null, previewColumnDayKey: preview?.previewColumnDayKey ?? sourceColumnDayKey }); }, [getDragPreviewTimes, onMoveCalendarEvent, setDragStateValue]);
  const handleAllDayEventPointerDown = useCallback((event: ReactPointerEvent<HTMLDivElement>, calendarEvent: GoogleCalendarEvent) => { startEventDrag(event, calendarEvent, 0, getEventTimedDragDurationMs(calendarEvent), null); }, [startEventDrag]);
  const handleTimedEventPointerDown = useCallback((event: ReactPointerEvent<HTMLDivElement>, calendarEvent: GoogleCalendarEvent, day: Date) => { const dayKey = getCalendarDateKey(day); const dayColumn = dayColumnRefs.current.get(dayKey); if (!dayColumn) return; const rowHeight = getHourRowHeightPx(dayColumn); const eventRect = event.currentTarget.getBoundingClientRect(); const pointerOffsetMinutes = Math.max(0, ((event.clientY - eventRect.top) / rowHeight) * GRID.WEEKDAY_MINUTES_PER_HOUR); startEventDrag(event, calendarEvent, pointerOffsetMinutes, getEventTimedDragDurationMs(calendarEvent), dayKey); }, [startEventDrag]);

  useEffect(() => { if (typeof window === "undefined") return undefined; const handlePointerMove = (event: PointerEvent) => updateDragPreview(event); const handlePointerUp = (event: PointerEvent) => finishDrag(event, true); const handlePointerCancel = (event: PointerEvent) => finishDrag(event, false); window.addEventListener("pointermove", handlePointerMove, { passive: false }); window.addEventListener("pointerup", handlePointerUp, { passive: false }); window.addEventListener("pointercancel", handlePointerCancel, { passive: false }); return () => { window.removeEventListener("pointermove", handlePointerMove); window.removeEventListener("pointerup", handlePointerUp); window.removeEventListener("pointercancel", handlePointerCancel); stopDragAutoScroll(); }; }, [finishDrag, stopDragAutoScroll, updateDragPreview]);
  useCalendarEventDragBodyStyle(Boolean(dragState));

  return (
    <div className="flex min-h-0 flex-1 flex-col overflow-hidden bg-white">
      <div ref={headerScrollRef} className="shrink-0 overflow-hidden border-b" style={{ borderColor: COLOR.WEEKDAY_COLOR_BORDER_MAIN }}><div className="grid min-w-0" style={{ gridTemplateColumns }}><div className="h-12" />{visibleDays.map((day) => { const dayKey = getCalendarDateKey(day); const isSelected = isSameCalendarDate(day, selectedDate); const isToday = isSameCalendarDate(day, now); return <div key={dayKey} className="flex h-12 min-w-0 items-center justify-center px-2"><button type="button" className="flex h-12 w-full min-w-0 items-center justify-center gap-1 bg-transparent p-0 text-center outline-none focus-visible:ring-2 focus-visible:ring-[#0a84ff]/25" aria-pressed={isSelected} onClick={() => onSelectDate?.(day)}><span className={getHeaderDateNumberClassName(isSelected, isToday)}>{format(day, "d", { locale: ja })}</span><span className={WEEKDAY_HEADER_WEEKDAY_CLASS_NAME}>{format(day, "EEE", { locale: ja })}</span></button></div>; })}</div></div>
      <div ref={allDayScrollRef} className="relative z-20 shrink-0 overflow-hidden border-b bg-white" style={WEEKDAY_COLUMN_BORDER_STYLE}><div className="grid min-w-0" style={{ gridTemplateColumns }}><div className={cn("flex min-h-10 min-w-0 items-start justify-end py-2 pl-2 pr-3", WEEKDAY_TIME_LABEL_CLASS_NAME)}>終日</div>{visibleDays.map((day, dayIndex) => { const dayKey = getCalendarDateKey(day); const events = createAllDayRenderItems(allDayEvents.get(dayKey) ?? [], dayKey, allDayEventOrder, dragState, dragPreviewEvent, dragPreviewDayKey); return <div key={dayKey} ref={setAllDayColumnRef(dayKey)} className={cn("min-h-10 min-w-0", dayIndex === 0 ? null : "border-l")} style={getAllDayColumnStyle()}><div className="flex min-w-0 flex-col" style={getAllDayStackStyle()}>{events.map(({ event, eventKey, isDragPreview }) => { const isDragging = dragState?.eventKey === eventKey; const isDraggable = isCalendarEventDraggable(event, onMoveCalendarEvent); return <div key={isDragPreview ? `${eventKey}:preview` : eventKey} data-calendar-all-day-event-item="true" className={getAllDayEventWrapperClassName(isDraggable, isDragging, isDragPreview)} style={isDragPreview ? CALENDAR_EVENT_DRAGGING_STYLE : undefined} onPointerDown={!isDragPreview && isDraggable ? (pointerEvent) => handleAllDayEventPointerDown(pointerEvent, event) : undefined}><CalendarEventChipMonth event={event} showTimeLabel={false} tooltipDisabled={isDragPreview || isDragging} /></div>; })}</div></div>; })}</div></div>
      <div ref={scrollContainerRef} className="calendar-timeline-scroll scrollbar-hidden -mt-2 min-h-0 flex-1 overflow-auto pt-2" onScroll={onScroll}><div className="relative grid min-w-0" style={timelineGridStyle}><div className="relative min-w-0 bg-white" style={{ zIndex: GRID.WEEKDAY_GRID_TIME_COLUMN_Z_INDEX }}>{WEEKDAY_HOURS.map((hour) => <div key={hour} className="relative" style={{ height: `var(${GRID.WEEKDAY_CSS_VAR_HOUR_ROW_HEIGHT})` }}><span className={getHourLabelClassName(hour)}>{formatHourLabel(hour)}</span></div>)}<div className={WEEKDAY_BOTTOM_TIME_SPACER_CLASS_NAME} data-testid="weekday-time-bottom-spacer" style={WEEKDAY_BOTTOM_SPACER_STYLE}><span className={getHourLabelClassName(GRID.WEEKDAY_HOURS)}>{formatHourLabel(GRID.WEEKDAY_HOURS)}</span></div></div>{visibleDays.map((day, dayIndex) => { const dayKey = getCalendarDateKey(day); const events = createTimedLayoutEvents(visibleEvents, day, minimumVisibleHeightPercent); const shouldRenderDragPreview = dragState && dragPreviewEvent && !dragState.previewIsAllDay && dragState.previewColumnDayKey === dayKey; return <div key={dayKey} ref={setDayColumnRef(dayKey)} className={cn("relative min-w-0 bg-white", dayIndex === 0 ? null : "border-l")} style={WEEKDAY_COLUMN_BORDER_STYLE}>{WEEKDAY_HOURS.map((hour) => <div key={hour} className="border-b" style={{ height: `var(${GRID.WEEKDAY_CSS_VAR_HOUR_ROW_HEIGHT})`, borderColor: COLOR.WEEKDAY_COLOR_BORDER_SUB }} />)}<div className={WEEKDAY_BOTTOM_PREVIEW_SPACER_CLASS_NAME} data-testid="weekday-preview-bottom-spacer" style={WEEKDAY_BOTTOM_SPACER_STYLE} />{events.map((entry) => { const eventKey = createCalendarEventKey(entry.event); const isDragging = dragState?.eventKey === eventKey; const isDraggable = isCalendarEventDraggable(entry.event, onMoveCalendarEvent); return <div key={eventKey} className={getTimedEventWrapperClassName(isDraggable, isDragging)} style={getTimedEntryPositionStyle(entry, events, WEEKDAY_TIMELINE_RANGE_HOURS)} onPointerDown={isDraggable ? (event) => handleTimedEventPointerDown(event, entry.event, day) : undefined}><CalendarEventChipWeekday event={entry.event} tooltipDisabled={isDragging} /></div>; })}{shouldRenderDragPreview ? <div className="absolute min-w-0 transition-none" style={getDragPreviewStyle(dragState, day)}><CalendarEventChipWeekday event={dragPreviewEvent} tooltipDisabled /></div> : null}</div>; })}{shouldRenderCurrentTime ? <div className="pointer-events-none absolute z-20" style={getCurrentTimeGridLineStyle(now)}><div className="h-px" style={{ backgroundImage: "repeating-linear-gradient(to right, rgb(59 130 246 / 0.45) 0 7px, transparent 7px 16px)" }} /><div className="absolute top-0 h-px bg-blue-500" style={getCurrentTimeTodayLineStyle(currentDayIndex, visibleDays.length)} /></div> : null}</div></div>
    </div>
  );
};
