...
const SOURCE_ROW_MARKER_CLASS_NAME = "relative flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-white transition-all duration-150";
const SOURCE_ROW_CHECKED_MARKER_BORDER_WIDTH = 6;
const SOURCE_ROW_UNCHECKED_MARKER_BORDER_WIDTH = 1;
const SOURCE_ROW_UNCHECKED_MARKER_COLOR = "#d9d9dc";
const SOURCE_ROW_UNCHECKED_MARKER_SHADOW = "0 1px 2px rgba(15, 23, 42, 0.12)";

const createSourceRowMarkerStyle = (color: string, checked: boolean): CSSProperties => ({
  backgroundColor: "#ffffff",
  borderColor: checked ? color : SOURCE_ROW_UNCHECKED_MARKER_COLOR,
  borderStyle: "solid",
  borderWidth: checked ? SOURCE_ROW_CHECKED_MARKER_BORDER_WIDTH : SOURCE_ROW_UNCHECKED_MARKER_BORDER_WIDTH,
  boxShadow: checked ? "none" : SOURCE_ROW_UNCHECKED_MARKER_SHADOW,
  boxSizing: "border-box",
} as CSSProperties);

const SourceRowMarker = ({ checked, color }: SourceRowMarkerProps) => {
  return <span className={SOURCE_ROW_MARKER_CLASS_NAME} style={createSourceRowMarkerStyle(color, checked)} aria-hidden="true" />;
};
