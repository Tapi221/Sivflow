const clampCardCarouselIndex = (index: number, itemCount: number) => { if (itemCount <= 0) { return 0;
  }
...
const canGoToPreviousCardCarouselItem = (activeIndex: number) => { return activeIndex > 0;
};
const canGoToNextCardCarouselItem = (activeIndex: number, itemCount: number) => { return activeIndex < Math.max(0, itemCount - 1);
};
