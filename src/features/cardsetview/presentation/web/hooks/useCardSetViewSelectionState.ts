...
export const useCardSetViewSelectionState = ({
  ...
  const [selectedCardIdState, setSelectedCardIdState] = useState<KeyedStringState>(() => ({
    sourceKey,
    value: null,
  }));
  ...
  const selectedCardId = selectedCardIdState.sourceKey === sourceKey ? selectedCardIdState.value : null;
  const pendingFocusCardId = pendingFocusState.sourceKey === sourceKey ? pendingFocusState.value : null;
  ...
  const currentIndexBase = pendingFocusIndex ?? selectedCardIndex ?? targetResolvedIndex ?? currentIndex ?? initialIndex;
  ...
  const { effectiveCard } = useCardEntity(isGlobalEditing ? (currentCard?.id ?? null) : null);
  ...
  const selectCardIndex = useCallback((index: number) => {
  ...
  const setCurrentIndex = useCallback((next: SetStateAction<number>) => {
    ...
    const resolved = typeof next === "function" ? (next as (prevState: number) => number)(prevValue) : next;
  ...
  const setCurrentCardFace = useCallback((face: CardFace) => {
    ...
    setFlippedState((prev) => {
      const baseIds = prev.sourceKey === sourceKey ? prev.ids : new Set<string>();
  ...
  const setInteractionMode = useCallback((mode: "view" | "edit") => {
