...
const isTypingTarget = (target: EventTarget | null) => { if (!isHTMLElement(target)) return false;
...
const isPrimaryShortcut = (event: KeyboardEvent, key: string) => { return hasPrimaryModifier(event) && !event.altKey && !event.shiftKey && event.key.toLowerCase() === key.toLowerCase();
};
const isPrimaryShiftShortcut = (event: KeyboardEvent, key: string) => { return hasPrimaryModifier(event) && !event.altKey && event.shiftKey && event.key.toLowerCase() === key.toLowerCase();
};
