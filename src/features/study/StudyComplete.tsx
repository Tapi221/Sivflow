...
export function StudyComplete({
  ...
  return (
    <div className={`animate-in fade-in duration-700 ${compact ? 'h-full min-h-0 flex flex-col' : ''}`}>
      <div className={compact ? 'mb-4 md:mb-5 shrink-0' : 'mb-8'}>
        <StampRally currentStreak={stampRallyStreak} compact={compact} />
      </div>

      <Card className={`max-w-3xl mx-auto border-none shadow-xl rounded-3xl overflow-hidden animate-in fade-in zoom-in duration-700 delay-300 ${compact ? 'w-full' : ''}`}>
        <CardContent className={`text-center relative overflow-hidden ${compact ? 'py-8 px-6 md:py-9 md:px-8' : 'py-12 px-8'}`}>
          <div className="absolute inset-0 bg-gradient-to-br from-slate-50 via-white to-[#e0f2f1] opacity-80"></div>
          <div className="relative z-10">
            <h2 className={`font-bold mb-2 text-[#1e293b] ${compact ? 'text-[30px] leading-none md:text-3xl' : 'text-2xl'}`}>Complete!</h2>
              ...
              {ratingTiles.map(({ rating }) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="text-lg font-bold text-[#1e293b]">{count}</span>
                    <span className={`text-[9px] text-[#94a3b8] font-medium uppercase tracking-wider ${design.labelHover}`}>{ratingLabels[rating]}</span>
                  </button>
                );
              })}
            </div>
