      ...
      {ORDER.map((key) => {
        ...
        return (
          ...
          >
            ...
            {singleColumn ? (
              <>
                <span className={`min-w-0 flex-1 truncate text-sm text-[#94a3b8] font-semibold ${disableHover ? '' : design.labelHover}`}>{design.label}</span>
                <span className="text-base font-bold text-[#1e293b] tabular-nums">{counts[key] ?? 0}</span>
              </>
            ) : (
              <>
