...
export const subscribeDesktopImportFileOpen = (handler: (payload: DesktopImportFileOpenPayload) => void | Promise<void>): (() => void) => {
...
export const readDesktopImportFile = async (filePath: string): Promise<File> => {
  if (!canUseDesktopImportFiles()) {
...
export const readDesktopImportFiles = async (filePaths: readonly string[]): Promise<File[]> => {
