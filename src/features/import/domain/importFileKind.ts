...
export const detectImportFileKind = (file: Pick<File, "name" | "type">): ImportFileKind => { const fileName = normalizeFileName(file.name);
...
export const isSupportedImportFileKind = (kind: ImportFileKind): kind is Exclude<ImportFileKind, "unknown"> => { return kind !== "unknown";
};
export const isPortableImportFileKind = (kind: ImportFileKind): kind is PortableImportFileKind => { return kind === "mfdeck" || kind === "mfcard";
};
export const isPortableImportFile = (file: Pick<File, "name" | "type">) => { return isPortableImportFileKind(detectImportFileKind(file));
};
