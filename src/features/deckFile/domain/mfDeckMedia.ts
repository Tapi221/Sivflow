...
const isMfDeckMediaUri = (value: unknown): value is string => { return (typeof value === "string" && value.startsWith(MF_DECK_MEDIA_URI_PREFIX));
};
const pathFromMfDeckMediaUri = (value: string): string | null => { if (!isMfDeckMediaUri(value)) return null;

  const path = value.slice(MF_DECK_MEDIA_URI_PREFIX.length);
  return isMfDeckMediaPath(path) ? path : null;
};
const inferMfDeckMediaKind = (mimeType: string): MfDeckMediaKindV1 => { if (mimeType.startsWith("image/")) return "image";
...
const sanitizeMfDeckMediaName = (value: string): string => { const sanitized = collapseRepeatedUnderscores(replaceWhitespaceWithUnderscore(replaceInvalidFileNameCharacters(replaceControlCharacters(value.trim())))).slice(0, 80);
