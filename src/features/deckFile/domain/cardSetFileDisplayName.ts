...
const stripMfDeckExtension = (value: string) => { return value.replace(/\.mfdeck$/iu, "").trim();
};
const stripCardSetDisplaySuffix = (value: string) => { return value.replace(CARD_SET_SUFFIX_PATTERN, "").trim();
};
const formatCardSetFileDisplayName = (rawName: string | null | undefined) => { const normalizedName = rawName?.trim() || "無題";
