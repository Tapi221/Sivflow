...
const getErrorStatus = (error: unknown): number | undefined => { if (!(error instanceof Error)) return undefined;
  return (error as Error & { status?: number; }).status;
};
const isUnauthorizedError = (error: unknown): boolean => getErrorStatus(error) === 401;
const getGoogleReason = (error: unknown): string | undefined => { if (!(error instanceof Error)) return undefined;
  return (error as Error & { googleReason?: string; }).googleReason;
};
const isGooglePermissionError = (error: unknown): boolean => { const status = getErrorStatus(error);
...
const getErrorCode = (error: unknown): string | undefined => { if (!(error instanceof Error)) return undefined;
  return (error as Error & { code?: string; }).code;
};
const normalizeErrorCode = (code: string | undefined): string | undefined => code?.replace(/^functions\//, "");
const getGoogleOAuthErrorReason = (error: unknown): GoogleOAuthCallableErrorReason | undefined => { const wrappedReason = error instanceof Error ? (error as Error & { googleOAuthReason?: GoogleOAuthCallableErrorReason; }).googleOAuthReason : undefined;
...
const isReconnectRequiredError = (error: unknown): boolean => { const code = normalizeErrorCode(getErrorCode(error));
...
const toGoogleCalendarAuthErrorMessage = (error: unknown): string => { const reason = getGoogleOAuthErrorReason(error);
...
const createGoogleOAuthCooldownError = (entry: GoogleOAuthCooldownEntry): Error => { const error = new Error(entry.message);
