...
export default function ImageDiagnostics() {
  ...
  const [summary, setSummary] = useState({ total: 0, valid: 0, error: 0, cors: 0 });
  ...
  const checkUrl = async (url, type, id, label) => {
    ...
    if (url.startsWith('blob:')) {
         return {
            id, type, label, url: 'Blob URL (ローカル)', status: 'ローカル', code: 200, isOk: true
         };
    }
    ...
    try {
      ...
      return {
        ...
        code: 0,
        isOk: false,
        errorType: error.message.includes('Failed to fetch') ? 'CORS/Network' : 'Other'
      };
    }
  };
  ...
  const runDiagnostics = async () => {
    ...
    try {
      ...
      try {
        const docRef = await getDoc(doc(firestoreDb, 'userSettings', currentUser.uid));
        if (docRef.exists()) {
            const data = docRef.data();
            if (data.profileImage?.remoteUrl) {
               const res = await checkUrl(data.profileImage.remoteUrl, 'プロフィール', docRef.id, data.displayName || 'ユーザー');
      ...
      for (const doc of cardsSnap.docs) {
        ...
        if (data.question_images && Array.isArray(data.question_images)) {
            for (let i = 0; i < data.question_images.length; i++) {
                ...
                const url = img?.remoteUrl || img?.url || (typeof img === 'string' ? img : null);
                if (url) {
                    const res = await checkUrl(url, 'カード (問題)', doc.id, `${data.title || 'タイトルなし'} #${i+1}`);
                    if (res) newResults.push(res);
                }
            }
        }
        ...
        if (data.answer_images && Array.isArray(data.answer_images)) {
            for (let i = 0; i < data.answer_images.length; i++) {
                ...
                const url = img?.remoteUrl || img?.url || (typeof img === 'string' ? img : null);
                if (url) {
                    const res = await checkUrl(url, 'カード (解答)', doc.id, `${data.title || 'タイトルなし'} #${i+1}`);
                    if (res) newResults.push(res);
                }
            }
        }
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
             <Card>
                <CardContent className="pt-6 text-center">
                    ...
                    <div className="text-xs text-slate-500 uppercase">チェック総数</div>
                </CardContent>
             </Card>
             <Card>
                <CardContent className="pt-6 text-center">
             ...
             <Card>
                <CardContent className="pt-6 text-center">
                    <div className="text-2xl font-bold text-amber-600">{summary.cors}</div>
                    <div className="text-xs text-slate-500 uppercase">CORS/ネットエラー</div>
                </CardContent>
             </Card>
        </div>
