...
export default function FourChoiceMode() {
  ...
  const [editors, setEditors] = useState(() => {
    const saved = sessionStorage.getItem(FOUR_CHOICE_EDITORS_KEY);
    if (saved) {
      try {
  ...
  useEffect(() => {
    sessionStorage.setItem(FOUR_CHOICE_EDITORS_KEY, JSON.stringify(editors));
  }, [editors]);

  const clearPersistence = () => {
    sessionStorage.removeItem(FOUR_CHOICE_EDITORS_KEY);
  };
  ...
  const saveWithRetry = async (cardData, editorId, maxRetries = 3) => {
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        console.log(`[FourChoiceMode] 保存試行 ${attempt}/${maxRetries} for editorId=${editorId}`);
        ...
        console.log('[FourChoiceMode] 保存成功:', result.id);
        return result;
      } catch (error) {
        console.error(`[FourChoiceMode] 保存失敗 (試行${attempt}):`, error);
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex flex-col">
            <h1 className="text-sm md:text-base font-bold text-slate-800 flex items-center gap-2">
              <span className="bg-green-100 text-green-700 px-2 py-0.5 rounded text-xs">4択問題</span>
              <span>作成モード</span>
            </h1>
                      ...
                      >
                        ...
                        <div className={cn(
                          ...
                          <CardEditor
                            ...
                            customDraftKey={`fourchoice_draft_${editor.id}`}
                            storageType="session"
                            mode="four_choice"
                            canMoveUp={index > 0}
