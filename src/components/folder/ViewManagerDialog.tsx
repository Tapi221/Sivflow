...
export function ViewManagerDialog(props: ViewManagerDialogProps) {
  const { open, onOpenChange, views, categoryNameEntries } = props;
...
function ViewManagerDialogBody(props: ViewManagerDialogBodyProps) {
  ...
  const [ungroupedLabelDrafts, setUngroupedLabelDrafts] = useState<
    Record<string, string>
  >(() =>
    Object.fromEntries(
      views.map((view) => [view.id, view.options?.ungroupedLabel ?? ""]),
    ),
  );
  const [categoryNameDrafts, setCategoryNameDrafts] = useState<
    Record<string, string>
  >(() => Object.fromEntries(categoryNameEntries));
  const [tagSearchDrafts, setTagSearchDrafts] = useState<
    Record<string, string>
  >({});

  const customViews = useMemo(
    () => views.filter((view) => view.kind !== "folder"),
    [views],
  );
  ...
  const commitUngroupedLabel = (viewId: string) => {
    ...
    const currentValue =
      views.find((view) => view.id === viewId)?.options?.ungroupedLabel ?? "";
  ...
  const commitCategoryName = (categoryId: string) => {
    ...
    const currentValue =
      categoryNameEntries.find(([id]) => id === categoryId)?.[1] ?? "";
    if (nextValue !== currentValue) {
      onUpdateCategoryName(categoryId, nextValue);
    }
  };

  const updateTagTreeOptions = (
    view: ViewDef,
    patch: Partial<NonNullable<ViewDef["options"]>>,
  ) => {
  ...
  const getScopeHelperText = (
    scopeMode: NonNullable<ViewDef["options"]>["scopeMode"],
  ) => {
    switch (scopeMode) {
      case "selectedRoots":
          ...
          <div className="flex items-end gap-2">
            <div className="flex-1 space-y-2">
              ...
              <select
                ...
                value={newViewKind}
                onChange={(event) =>
                  setNewViewKind(event.target.value as ViewKind)
                }
            ...
            <Button type="button" onClick={() => onAddView(newViewKind)}>
              追加
            </Button>
          </div>
            ...
            {customViews.map((view) => (
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  />
                  <div className="min-w-24 text-xs text-slate-500">
                    {view.kind}
                  </div>
                  <Button
                    type="button"
                  ...
                  <div className="mt-3 space-y-3">
                    <div className="space-y-2">
                      ...
                      <select
                        ...
                        onChange={(event) => {
                          const nextScopeMode = event.target
                            .value as NonNullable<
                            ViewDef["options"]
                          >["scopeMode"];
                          updateTagTreeOptions(view, {
                    ...
                    {(view.options?.scopeMode ?? "all") === "selectedRoots" ? (
                      <div className="space-y-2">
                        <Label htmlFor={`root-tag-search-${view.id}`}>
                          起点ルートタグ
                        </Label>
                        <Input
                        ...
                        {(view.options?.rootTagIds?.length ?? 0) > 0 ? (
                          <div className="flex flex-wrap gap-2 rounded-md border border-emerald-200 bg-emerald-50/60 p-2">
                            {(view.options?.rootTagIds ?? []).map(
                              (rootTagId) => (
                                <span
                                  key={rootTagId}
                                  className="rounded-full border border-emerald-200 bg-white px-2 py-1 text-xs text-emerald-700 surface-convex"
                                >
                          ...
                          {filterTags(view.id).map((tag) => {
                            ...
                            return (
                              <label
                                ...
                                className={
                                  checked
                                    ? "flex items-center gap-2 rounded-md bg-emerald-50 px-2 py-1 text-sm text-slate-700"
                                    : "flex items-center gap-2 rounded-md px-2 py-1 text-sm text-slate-700"
                                }
                              >
                                <input
                                  type="checkbox"
                                  ...
                                  onChange={(event) => {
                                    const nextRootTagIds = event.target.checked
                                      ? [...rootTagIds, tag.id]
                                      : rootTagIds.filter(
                                          (rootTagId) => rootTagId !== tag.id,
                                        );
                                    updateTagTreeOptions(view, {
                                      rootTagIds: nextRootTagIds,
                                    });
                                  }}
                                />
                    ...
                    {(view.options?.scopeMode ?? "all") === "selectedTags" ? (
                      <div className="space-y-2">
                        <Label htmlFor={`tag-search-${view.id}`}>
                          含めるタグ
                        </Label>
                        <Input
                            ...
                            return (
                              ...
                              >
                                <input
                                  ...
                                  checked={checked}
                                  onChange={(event) => {
                                    const nextIncludedTagIds = event.target
                                      .checked
                                    ...
                                    updateTagTreeOptions(view, {
                                      includedTagIds: nextIncludedTagIds,
                                    });
                                  }}
                                />
                                <span className="truncate">{tag.name}</span>
                    ...
                    {(view.options?.scopeMode ?? "all") === "prefix" ? (
                      <div className="space-y-2">
                        ...
                        <Input
                          ...
                          onChange={(event) =>
                            updateTagTreeOptions(view, {
                              tagNamePrefix: event.target.value,
                            })
                          }
                        />
                        <div className="space-y-1 text-xs text-slate-500">
                          ...
                          <p>
                            例）`JavaScript/` と入力すると `JavaScript/...`
                            配下だけ表示します。
                          </p>
                          <p>空欄なら全て表示します。</p>
                        </div>
                    ...
                    <label className="flex items-center gap-2 text-sm text-slate-700">
                      <input
                        ...
                        onChange={(event) =>
                          updateTagTreeOptions(view, {
                            hideZeroUsage: event.target.checked,
                          })
                        }
                      />
                      <span>0件タグを隠す</span>
