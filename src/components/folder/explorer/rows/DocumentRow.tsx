...
const DocumentRow = ({ treeNode, style, depth, isSelected, editingId, editingName, renameCancelledRef, editInputRef, setEditingId, setEditingName, openRowMenuId, setOpenRowMenuId, onItemSelect, canRename, canDelete, handleDelete, handleRenameConfirm, setRowRef }: DocumentRowProps) => { const rowMenuId = `document:${treeNode.rawId}`;
  ...
  return (
    <SidebarEntityRow
      ...
      onMenuOpenChange={(open) => { setOpenRowMenuId(open ? rowMenuId : null); }}
      menuActions={rowMenuActions}
      hasContextMenu={rowMenuActions.length > 0}
      isEditing={isEditing}
      onContextMenuSelect={() => { onItemSelect({ type: "document", id: treeNode.rawId }); }}
      ...
      input={<input ref={attachEditInputRef} aria-label="文書名の編集" className={EXPLORER_ROW_INPUT_CLASS} defaultValue={editingName} onFocus={(e) => { e.currentTarget.select(); }} onMouseUp={(e) => { e.preventDefault(); }} onChange={(e) => { setEditingName(e.target.value); }} onClick={(e) => e.stopPropagation()} onKeyDown={(e) => { const isComposing = e.nativeEvent.isComposing || e.keyCode === 229; if (e.key === "Enter" && isComposing) return; if (e.key === "Enter") { e.preventDefault(); setEditingName(e.currentTarget.value); e.currentTarget.blur(); } if (e.key === "Escape") { e.preventDefault(); e.stopPropagation(); renameCancelledRef.current = true; e.currentTarget.blur(); } }} onBlur={(e) => { setEditingName(e.currentTarget.value); void handleRenameConfirm({ id: treeNode.rawId, type: "document" }); }} />}
