...
export default function BulkTagDialog({
  ...
  const handleApply = async () => {
    ...
    setResult(null);
    try {
      const count = await addTagToCardsInFolder(folderId, selectedTagId, includeSubfolders);
      setResult(count);
    } finally {
      setIsWorking(false);
    }
      ...
      <DialogContent className="max-w-md">
        ...
        {folderName && (
          <p className="text-sm text-slate-500 -mt-2">
            フォルダ: <span className="font-medium text-slate-700">{folderName}</span>
          </p>
        )}
        ...
        <div className="mt-2">
          ...
          {tags.length === 0 ? (
            <p className="text-sm text-slate-400">タグがまだありません。先にタグを作成してください。</p>
          ) : (
