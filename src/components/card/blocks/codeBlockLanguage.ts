...
export function normalizeViewerLanguage(input?: string): string {
  const raw = (input || "").toLowerCase().trim();
  const normalized = VIEWER_LANGUAGE_ALIASES[raw] ?? raw;
  return VIEWER_SUPPORTED_LANGS.has(normalized) ? normalized : "clike";
}

export function getViewerLanguageLabels(language: string) {
  return {
    full: VIEWER_LANGUAGE_FULL_LABELS[language] ?? language,
    short: VIEWER_LANGUAGE_SHORT_LABELS[language] ?? language.toUpperCase(),
  };
}

export function normalizeEditorLanguage(input?: string): string {
