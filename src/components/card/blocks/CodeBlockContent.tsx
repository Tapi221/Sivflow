...
export function CodeBlockContent(props: CodeBlockContentProps) {
  ...
  const normalizedCode = useMemo(() => (props.code ?? '').replace(/\s+$/, ''), [props.code]);
  ...
  if (props.mode === 'viewer') {
    return (
      <div className={props.className}>
        <CodeBlockFrame languageLabel={labels.short} languageTitle={labels.full} headerRight={copyButton}>
          <Highlight theme={codeTheme} code={normalizedCode} language={viewerLanguage}>
            {({ className: preClassName, style, tokens, getLineProps, getTokenProps }: RenderProps) => {
              ...
              const visibleTokens = lastLineIsTrailingEmpty ? tokens.slice(0, -1) : tokens;
  ...
  const highlightCode = (src: string) => Prism.highlight(src, editorGrammar, editorLanguage);

  return (
    <div ref={editorHostRef} className={props.className}>
      <CodeBlockFrame variant="editor" headerLeft={props.headerLeft} headerRight={copyButton}>
        <Editor
          value={props.code}
