...
export const isEditorInsertableBlockType = (type: CardBlock["type"]): type is Exclude<CardBlock["type"], "audio" | "reference"> => {
  return !EDITOR_NON_INSERTABLE_BLOCK_TYPES.has(type);
};
export const createEditorBlock = ({ prefix: _prefix, type, id, rowOffset, offsetRows, parentBlockId }: CreateEditorBlockParams): CardBlock => {
