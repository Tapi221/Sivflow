...
const saveViewerStateToSession = (docId: string, state: PdfViewerState): void => {
  try {
    const key = `pdf_viewer_${docId}`;
...
export function PdfPane({ doc, className, viewerOptions, onDocumentUpdate }: PdfPaneProps) {
  ...
  const [localDataStatus, setLocalDataStatus] = useState<'idle' | 'loading' | 'ready' | 'failed'>('idle');
  const [failedRemoteSourceKey, setFailedRemoteSourceKey] = useState<string | null>(null);
  const [failedBlobUrl, setFailedBlobUrl] = useState<string | null>(null);

  const remoteUrl = useMemo(() => {
    const candidate = doc.remoteUrl ?? doc.downloadUrl ?? null;
    if (typeof candidate === 'string' && candidate.startsWith('blob:')) return null;
    return candidate;
  ...
  const localBlobId = useMemo(() => doc.localFileId ?? doc.id ?? null, [doc.id, doc.localFileId]);
  const effectiveRemoteUrl = useMemo(() => {
    if (!remoteUrl) return null;
    if (failedRemoteSourceKey && remoteSourceKey === failedRemoteSourceKey) return null;
    return remoteUrl;
  ...
  const fitScale = useMemo(() => {
    ...
    return clamp(Number((usableWidth / basePageWidth).toFixed(3)), FIT_MIN_SCALE, FIT_MAX_SCALE);
  ...
  useEffect(() => {
    ...
    const nextCached = getCachedDocumentBlobUrl(localBlobId, { userId: currentUser?.uid });
  ...
  useEffect(() => {
    ...
    if (restoredState) {
      ...
      if (restoredState.fitMode === 'width' || restoredState.fitMode === 'manual') {
  ...
  useEffect(() => {
    ...
    getDocumentBlob(localBlobId, { userId: currentUser?.uid })
      .then(async (blob) => {
        ...
        cacheDocumentBlobUrl(localBlobId, nextBlobUrl, { userId: currentUser?.uid });
  ...
  const localBlobUrl = usableCachedBlobUrl ?? usableRestoredBlobUrl ?? usablePersistedBlobUrl;
    ...
    debounceTimerRef.current = setTimeout(() => {
      ...
      if (onDocumentUpdate) {
        onDocumentUpdate({ viewerState: newViewerState }).catch((err) => {
          console.warn('[PdfPane] Failed to save viewer state:', err, { docId: doc.id });
          // 失敗しても無視（UXを止めない）
        });
      }
  ...
  const handleSourceLoadError = useCallback(
    (details: { kind: 'remote-url' | 'blob-url' | 'data' | 'unknown'; url: string | null; message: string }) => {
      if (details.kind === 'remote-url' && remoteSourceKey) {
        ...
        console.warn('[PdfPane] Remote source failed, fallback to local source', {
          docId: doc.id,
          remoteUrl,
          remoteSourceKey,
          localFileId: localBlobId,
      ...
      if (details.kind === 'blob-url' && details.url) {
        ...
        console.warn('[PdfPane] blob source failed, retrying from localFileId restore', {
          docId: doc.id,
          blobUrl: details.url,
          localFileId: localBlobId,
          message: details.message,
        ...
        invalidateDocumentBlobUrl(localBlobId, details.url, { userId: currentUser?.uid });
  ...
  const handleZoomOut = () => {
    setFitMode('manual');
    setScale((s) => clamp(parseFloat((s - ZOOM_STEP).toFixed(2)), FIT_MIN_SCALE, FIT_MAX_SCALE));
  };
  ...
  const handleFirstPageSize = useCallback((size: { width: number; height: number } | null) => {
    ...
    <div className={cn("flex flex-col h-full min-h-0 min-w-0", className)}>
      <div className="flex items-center justify-between gap-2 px-3 py-2 border-b border-slate-200 bg-white">
        <div className="min-w-0">
          ...
          {isLocalOnly && (
            <div className="text-[11px] text-amber-600">このPDFはこの端末ローカルのみです（クラウド未同期）。</div>
          )}
          {doc.uploadStatus === 'failed' && (
            <div className="text-[11px] text-rose-600">クラウド同期に失敗しました。再アップロードを試してください。</div>
          )}
        </div>
      ...
      >
        {sourceUnavailable ? (
          <div className="text-sm text-slate-500 p-4">
            ...
            {localDataStatus === 'failed' && 'ローカルファイルが見つかりません。再アップロードしてください。'}
