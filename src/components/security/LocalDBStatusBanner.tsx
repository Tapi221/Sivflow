...
export function LocalDBStatusBanner() {
  ...
  const message = status.mode === 'fallback'
    ...
    <div className="border-b border-amber-300 bg-amber-50 px-4 py-3 text-amber-900">
      <div className="flex flex-wrap items-start gap-3 text-sm">
        <div className="mt-0.5 flex items-center gap-2 font-bold">
          {status.mode === 'fallback' ? <AlertTriangle size={18} /> : <Database size={18} />}
          <span>ストレージ警告</span>
        </div>
        <div className="flex-1 min-w-[240px]">
          <p>{message}</p>
          {status.resetFailedReason && (
            <p className="mt-1 text-xs opacity-80 break-all">詳細: {status.resetFailedReason}</p>
          )}
          <a
