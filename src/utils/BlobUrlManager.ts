...
class BlobUrlManager {
  ...
  create(blob: Blob): string {
    ...
    if (this.activeUrls.size >= this.MAX_ACTIVE_URLS) {
      console.warn(`[BlobUrlManager] Max URLs reached (${this.MAX_ACTIVE_URLS}). Revoking oldest...`);
    ...
    console.log(`[BlobUrlManager] Created: ${url.substring(0, 50)}... (Total: ${this.activeUrls.size})`);
    return url;
  }
  ...
  revoke(url: string): void {
    if (this.activeUrls.has(url)) {
      ...
      console.log(`[BlobUrlManager] Revoked: ${url.substring(0, 50)}... (Remaining: ${this.activeUrls.size})`);
    }
  }
