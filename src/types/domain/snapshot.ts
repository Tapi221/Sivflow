...
/** ユーザー設定 */
interface UserSettings { dailyGoal: number;
  notificationsEnabled: boolean;
}
/** スナップショットに含める画像アセット manifest */
interface SnapshotAsset { assetId: string;
  storagePath: string;
  mime: string;
...
/** スナップショットのデータ本体 */
interface SnapshotData { cards: Card[];
  cardSets: CardSet[];
  folders: Folder[];
  reviews: ReviewLog[];
...
/** アプリケーション全体のスナップショット */
interface AppSnapshot { metadata: SnapshotMetadata;
  data: SnapshotData;
}
/** スナップショット比較結果 */
interface SnapshotComparison { /** どちらが新しいか */ newerSnapshot: "local" | "imported" | "same";
