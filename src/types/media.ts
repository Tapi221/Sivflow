...
export const imageToMediaAsset = (image: any): MediaAsset => {
  return {
    ...
    status: image.status === 'uploading' ? 'uploading' : 
            image.status === 'ready' ? 'ready' : 
            image.status === 'pending' ? 'pending' : 'failed',
    contentType: image.contentType,
    sizeBytes: image.sizeBytes || image.size,
    storagePath: image.storagePath,
    checksum: image.checksum,
    uploadState: image.status === 'uploading' ? 'inProgress' :
                 image.status === 'ready' ? 'completed' :
                 image.status === 'pending' ? 'pending' : 'failed',
...
export const mediaAssetToImage = (asset: MediaAsset): any => {
  ...
  return {
    ...
    status: asset.status === 'uploading' ? 'uploading' :
            asset.status === 'ready' ? 'ready' :
            asset.status === 'pending' ? 'pending' : 'failed',
    contentType: asset.contentType,
    sizeBytes: asset.sizeBytes,
    storagePath: asset.storagePath,
    checksum: asset.checksum,
    uploadState: asset.status === 'uploading' ? 'inProgress' :
                 asset.status === 'ready' ? 'completed' :
                 asset.status === 'pending' ? 'pending' : 'failed',
