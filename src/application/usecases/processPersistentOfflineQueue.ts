...
const processPersistentOfflineQueue = async (items: QueueItem[], deps: PersistentOfflineQueueProcessingDeps): Promise<void> => { for (const item of items) { try { try { const shouldSkip = await deps.shouldSkipItem(item);
  if (shouldSkip) {
