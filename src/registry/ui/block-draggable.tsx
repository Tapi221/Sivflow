...
import { type PlateEditor, type PlateElementProps, type RenderNodeWrapper, MemoizedChildren, useEditorRef, useElement, usePluginOption, useSelected } from 'platejs/react';
...
const calculatePreviewTop = (editor: PlateEditor, { blocks, element }: { blocks: TElement[]; element: TElement }): number => {
...
const calcDragButtonTop = (editor: PlateEditor, element: TElement): number => {
  ...
  const currentMarginTopString = window.getComputedStyle(child).marginTop;
  const currentMarginTop = Number(currentMarginTopString.replace('px', ''));
  const currentLineHeightString = window.getComputedStyle(child).lineHeight;
  const currentLineHeight = Number(currentLineHeightString.replace('px', ''));

  return currentMarginTop + currentLineHeight / 2 - 12;
};
...
const MemoizedDropLine = React.memo(DropLine);

function DragHandle({ isDragging, previewRef, resetPreview, setPreviewTop }: { isDragging: boolean; previewRef: React.RefObject<HTMLDivElement | null>; resetPreview: () => void; setPreviewTop: (top: number) => void }) {
...
const MemoizedDragHandle = React.memo(DragHandle);
...
function Draggable(props: PlateElementProps) {
  ...
  return (
    <div
      className={cn('relative', isDragging && 'opacity-50', getPluginByType(editor, element.type)?.node.isContainer ? 'group/container' : 'group')}
      onMouseEnter={() => {
        if (isDragging) return;
    ...
    >
        ...
        <Gutter>
          <div className={cn('slate-blockToolbarWrapper', 'flex h-[1.5em]', isInColumn && 'h-4')}>
            <div className={cn('slate-blockToolbar relative w-4.5', 'pointer-events-auto mr-1 flex items-center', isInColumn && 'mr-1.5')}>
              <Button ref={handleRef} variant="ghost" className="-left-0 absolute h-6 w-full p-0" style={{ top: `${dragButtonTop + 3}px` }} data-plate-prevent-deselect>
                <MemoizedDragHandle isDragging={isDragging} previewRef={previewRef} resetPreview={resetPreview} setPreviewTop={setPreviewTop} />
              </Button>
            </div>
          </div>
        </Gutter>
      ...
      <div ref={previewRef} className={cn('-left-0 absolute hidden w-full')} style={{ top: `${-previewTop}px` }} contentEditable={false} />
