...
const DropdownMenuItem = ({ className, inset, variant = "default", ...props }: React.ComponentProps<typeof DropdownMenuPrimitive.Item> & { inset?: boolean; variant?: "default" | "destructive"; }) => {
  return (
    <DropdownMenuPrimitive.Item
      className={cn(
        "relative flex cursor-default select-none items-center gap-2 rounded-sm px-2 py-1.5 text-sm text-[#1f1f1f] outline-hidden focus:bg-[#eee] focus:text-[#1f1f1f] data-[disabled]:pointer-events-none data-[disabled]:text-[rgba(0,0,0,0.35)] data-[inset]:pl-8 data-[variant=destructive]:text-destructive data-[variant=destructive]:focus:bg-destructive/10 data-[variant=destructive]:focus:text-destructive dark:data-[variant=destructive]:focus:bg-destructive/20 [&_svg:not([class*='size-'])]:size-4 [&_svg:not([class*='text-'])]:text-[rgba(0,0,0,0.55)] [&_svg]:pointer-events-none [&_svg]:shrink-0 data-[variant=destructive]:*:[svg]:text-destructive!",
        className,
      )}
      data-inset={inset}
...
const DropdownMenuCheckboxItem = ({ className, children, checked, ...props }: React.ComponentProps<typeof DropdownMenuPrimitive.CheckboxItem>) => {
  return (
    <DropdownMenuPrimitive.CheckboxItem
      checked={checked}
      className={cn(
        "relative flex cursor-default select-none items-center gap-2 rounded-sm py-1.5 pr-2 pl-8 text-sm text-[#1f1f1f] outline-hidden focus:bg-[#eee] focus:text-[#1f1f1f] data-[disabled]:pointer-events-none data-[disabled]:text-[rgba(0,0,0,0.35)] [&_svg:not([class*='size-'])]:size-4 [&_svg]:pointer-events-none [&_svg]:shrink-0",
        className,
      )}
...
const DropdownMenuLabel = ({ className, inset, ...props }: React.ComponentProps<typeof DropdownMenuPrimitive.Label> & { inset?: boolean; }) => {
  return (
    <DropdownMenuPrimitive.Label
      className={cn("px-2 py-1.5 font-medium text-[rgba(0,0,0,0.55)] text-sm data-[inset]:pl-8", className)}
      data-inset={inset}
...
const DropdownMenuSeparator = ({ className, ...props }: React.ComponentProps<typeof DropdownMenuPrimitive.Separator>) => {
  return <DropdownMenuPrimitive.Separator className={cn("-mx-1 my-1 h-px bg-[rgba(0,0,0,0.08)]", className)} data-slot="dropdown-menu-separator" {...props} />;
};
const DropdownMenuShortcut = ({ className, ...props }: React.ComponentProps<"span">) => {
  return <span className={cn("ml-auto text-[rgba(0,0,0,0.55)] text-xs tracking-widest", className)} data-slot="dropdown-menu-shortcut" {...props} />;
};
...
const DropdownMenuSubTrigger = ({ className, inset, children, ...props }: React.ComponentProps<typeof DropdownMenuPrimitive.SubTrigger> & { inset?: boolean; }) => {
  return (
    <DropdownMenuPrimitive.SubTrigger
      className={cn(
        "flex cursor-default select-none items-center gap-2 rounded-sm px-2 py-1.5 text-sm text-[#1f1f1f] outline-hidden focus:bg-[#eee] focus:text-[#1f1f1f] data-[state=open]:bg-[#eee] data-[inset]:pl-8 data-[state=open]:text-[#1f1f1f] [&_svg:not([class*='size-'])]:size-4 [&_svg:not([class*='text-'])]:text-[rgba(0,0,0,0.55)] [&_svg]:pointer-events-none [&_svg]:shrink-0",
        className,
      )}
      data-inset={inset}
