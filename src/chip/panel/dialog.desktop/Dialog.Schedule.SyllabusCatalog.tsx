...
const ScheduleSyllabusCatalogDialog = ({ activeSemesterId, institutions, periods, syllabusCourses, onSearch, onSaveSyllabusCourse, onAddCourseFromSyllabus, onClose }: ScheduleSyllabusCatalogDialogProps) => {
  ...
  return (
    <DialogDesktopPanel surfaceClassName="flex max-h-[calc(100dvh-100px)] w-full max-w-96 flex-col" ariaLabel="シラバス検索・登録" onClose={onClose}>
      <div className="flex shrink-0 items-center justify-between border-b border-[#f0f0f2] px-5 py-4">
        <div className="min-w-0">
          <h2 className="text-base font-bold text-zinc-900">シラバス</h2>
          <p className="mt-1 text-xs font-medium text-zinc-500">大学・学部・授業を検索して時間割に追加</p>
        </div>
      </div>
      <div className="grid min-h-0 flex-1 grid-cols-1 overflow-hidden lg:grid-cols-[360px_minmax(0,1fr)]">
        <div className="min-h-0 overflow-y-auto border-b border-[#f0f0f2] px-5 py-4 lg:border-b-0 lg:border-r">
          <div className="mb-4 flex items-center justify-between">
            <div className="text-xs font-bold text-zinc-900">授業を登録</div>
            <div className="rounded-full bg-zinc-50 px-2 py-1 text-xs font-bold text-zinc-500">{catalogCountLabel}</div>
          </div>
          ...
          <div className="grid gap-2">
            ...
            <><div className="mb-2 flex items-center justify-between"><span className="text-xs font-bold text-zinc-500">曜日・時限</span><button type="button" className="rounded-full px-2 py-1 text-xs font-bold text-blue-500 disabled:text-[#c7c7cc]" disabled={periods.length === 0} onClick={handleAddSlot}>追加</button></div><div className="grid gap-2">{slots.map((slot, index) => <div key={index} className="grid grid-cols-[minmax(0,1fr)_minmax(0,1fr)_auto] gap-2"><select className="h-9 rounded-xl border px-2 text-xs font-bold" value={slot.dayIndex} onChange={(event) => { const nextDayIndex = Number(event.target.value); if (isValidCalendarTimetableWeekdayIndex(nextDayIndex)) handleUpdateSlot(index, { ...slot, dayIndex: nextDayIndex }); }} aria-label="曜日">{TIMETABLE_DAY_LABELS.map((label, dayIndex) => <option key={label} value={dayIndex}>{label}</option>)}</select><select className="h-9 rounded-xl border px-2 text-xs font-bold" value={slot.periodLabel} onChange={(event) => handleUpdateSlot(index, { ...slot, periodLabel: event.target.value })} aria-label="時限">{periods.map((period) => <option key={period.id} value={period.label}>{period.label}限</option>)}</select><button type="button" className="rounded-full px-2 text-xs font-bold text-[#ff3b30]" onClick={() => handleDeleteSlot(index)}>削除</button></div>)}</div></>
        ...
        <div className="flex min-h-0 flex-col overflow-hidden px-5 py-4">
          <div className="mb-3 flex gap-2"><input className="h-10 min-w-0 flex-1 rounded-xl border px-3 text-xs font-semibold" value={query} onChange={(event) => setQuery(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") handleSearch(); }} placeholder="大学名・学部・授業名・教員で検索" /><button type="button" className="h-10 rounded-full border px-4 text-xs font-bold text-blue-500" onClick={handleSearch}>検索</button></div>
          <div className="min-h-0 flex-1 overflow-y-auto">
            <div className="grid gap-2">
              {results.map((course) => <div key={course.id} className="rounded-2xl border border-slate-200 bg-[#fafafa] p-3"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><div className="truncate text-sm font-bold text-zinc-900">{course.title}</div><div className="mt-1 truncate text-xs font-semibold text-zinc-500">{course.institutionName} / {course.facultyName}{course.facultyName && course.departmentName ? "・" : ""}{course.departmentName}</div><div className="mt-1 truncate text-xs font-medium text-zinc-500">{course.teacher ?? "教員未設定"} / {course.room ?? "教室未設定"} / {getSyllabusCourseSlotsLabel(course)}</div></div><button type="button" className="shrink-0 rounded-full bg-[#007aff] px-3 py-1.5 text-xs font-bold text-white" onClick={() => handleAddCourse(course)}>追加</button></div>{course.memo ? <div className="mt-2 line-clamp-2 text-xs font-medium text-zinc-500">{course.memo}</div> : null}</div>)}
            </div>
            {results.length === 0 ? <div className="rounded-2xl border border-dashed px-4 py-8 text-center text-xs font-semibold text-zinc-500">シラバスはまだありません。</div> : null}
          </div>
        </div>
      </div>
    </DialogDesktopPanel>
