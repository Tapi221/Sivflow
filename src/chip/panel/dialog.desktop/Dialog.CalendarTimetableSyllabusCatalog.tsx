...
const CalendarTimetableSyllabusCatalogDialog = ({ activeSemesterId, institutions, periods, syllabusCourses, onSearch, onSaveSyllabusCourse, onAddCourseFromSyllabus, onClose }: CalendarTimetableSyllabusCatalogDialogProps) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/20 px-4 py-6" role="dialog" aria-modal="true" aria-label="授業DB">
      <div className="flex max-h-full w-full max-w-96 flex-col overflow-hidden rounded-3xl border border-[#e5e5ea] bg-white shadow-[0_18px_60px_rgba(0,0,0,0.18)]">
        <div className="flex shrink-0 items-center justify-between border-b border-[#f0f0f2] px-5 py-4">
          <div className="min-w-0"><h2 className="text-[17px] font-bold text-[#1c1c1e]">授業DB</h2><p className="mt-1 text-xs font-medium text-zinc-500">大学・学部・授業を登録して時間割に追加</p></div>
          <button type="button" className="rounded-full px-3 py-1.5 text-xs font-semibold text-[#6e6e73] hover:bg-[#f7f7f8]" onClick={onClose}>閉じる</button>
        </div>
        <div className="grid min-h-0 flex-1 grid-cols-1 overflow-hidden lg:grid-cols-[360px_minmax(0,1fr)]">
          <div className="min-h-0 overflow-y-auto border-b border-[#f0f0f2] px-5 py-4 lg:border-b-0 lg:border-r">
            <div className="mb-4 flex items-center justify-between"><div className="text-xs font-bold text-[#1c1c1e]">授業を登録</div><div className="rounded-full bg-[#f7f7f8] px-2 py-1 text-xs font-bold text-zinc-500">{catalogCountLabel}</div></div>
            {institutions.length > 0 ? <div className="mb-3 flex flex-wrap gap-2">{institutions.slice(0, 8).map((institution) => <button key={institution.id} type="button" className="rounded-full border border-[#e5e5ea] px-2.5 py-1 text-xs font-bold text-[#6e6e73]" onClick={() => setInstitutionName(institution.name)}>{institution.name}</button>)}</div> : null}
          ...
          <div className="flex min-h-0 flex-col overflow-hidden px-5 py-4">
            ...
            <div className="min-h-0 flex-1 overflow-y-auto"><div className="grid gap-2">{results.map((course) => <div key={course.id} className="rounded-[16px] border border-slate-200 bg-[#fafafa] p-3"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><div className="truncate text-sm font-bold text-[#1c1c1e]">{course.title}</div><div className="mt-1 truncate text-xs font-semibold text-[#6e6e73]">{course.institutionName} / {course.facultyName}{course.facultyName && course.departmentName ? "・" : ""}{course.departmentName}</div><div className="mt-1 truncate text-xs font-medium text-zinc-500">{course.teacher ?? "教員未設定"} / {course.room ?? "教室未設定"} / {getSyllabusCourseSlotsLabel(course)}</div></div><button type="button" className="shrink-0 rounded-full bg-[#007aff] px-3 py-1.5 text-xs font-bold text-white" onClick={() => handleAddCourse(course)}>追加</button></div>{course.memo ? <div className="mt-2 line-clamp-2 text-xs font-medium text-[#6e6e73]">{course.memo}</div> : null}</div>)}</div>{results.length === 0 ? <div className="rounded-[16px] border border-dashed px-4 py-8 text-center text-xs font-semibold text-zinc-500">授業DBは空です。</div> : null}</div>
          </div>
        </div>
      </div>
    </div>
