    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/20 px-4 py-6" role="dialog" aria-modal="true" aria-label="授業編集">
      <div className="flex max-h-full w-full max-w-96 flex-col overflow-hidden rounded-3xl border border-[#e5e5ea] bg-white shadow-[0_18px_60px_rgba(0,0,0,0.18)]">
        <div className="flex shrink-0 items-center justify-between border-b border-[#f0f0f2] px-5 py-4">
          <h2 className="text-[17px] font-bold tracking-tight text-[#1c1c1e]">{course ? "授業を編集" : "授業を追加"}</h2>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-5 py-4">
          <div className="grid gap-3">
            <input className="h-11 rounded-2xl border border-[#e5e5ea] px-3 text-sm font-semibold text-[#1c1c1e] outline-none focus:border-[#007aff]" value={title} onChange={(event) => setTitle(event.target.value)} placeholder="授業名（空欄なら授業）" />
            <div className="grid grid-cols-2 gap-3">
              <input className="h-10 rounded-xl border border-[#e5e5ea] px-3 text-sm font-semibold text-[#1c1c1e] outline-none focus:border-[#007aff]" value={room} onChange={(event) => setRoom(event.target.value)} placeholder="教室" />
              <input className="h-10 rounded-xl border border-[#e5e5ea] px-3 text-sm font-semibold text-[#1c1c1e] outline-none focus:border-[#007aff]" value={teacher} onChange={(event) => setTeacher(event.target.value)} placeholder="担当教員" />
            </div>
            <textarea className="min-h-16 rounded-xl border border-[#e5e5ea] px-3 py-2 text-sm font-medium text-[#1c1c1e] outline-none focus:border-[#007aff]" value={memo} onChange={(event) => setMemo(event.target.value)} placeholder="メモ" />
            <>
              <div className="mb-2 text-xs font-bold text-[#6e6e73]">色</div>
            ...
            <>
              <div className="mb-2 text-xs font-bold text-[#6e6e73]">曜日・時限</div>
              <div className="grid gap-2" style={{ gridTemplateColumns: `52px repeat(${visibleDayCount}, minmax(0, 1fr))` }}>
                <div />
                {TIMETABLE_DAY_LABELS.slice(0, visibleDayCount).map((label) => <div key={label} className="text-center text-xs font-bold text-[#6e6e73]">{label}</div>)}
                {periods.map((period) => <div key={period.id} className="contents"><div className="flex items-center justify-end pr-2 text-xs font-bold text-[#6e6e73]">{period.label}</div>{TIMETABLE_DAY_LABELS.slice(0, visibleDayCount).map((_, dayIndex) => {
                  ...
                  return <button key={`${dayIndex}-${period.id}`} type="button" className={cn("flex h-9 items-center justify-center rounded-xl border text-xs font-bold", selected ? "border-[#007aff] bg-[#e8f2ff] text-blue-500" : "border-[#e5e5ea] bg-white text-[#c7c7cc]")} onClick={() => toggleSlot(slot)}>{selected && StratisCheckIcon ? <StratisCheckIcon className="h-4 w-4" aria-hidden="true" focusable="false" /> : null}</button>;
                })}</div>)}
              </div>
            </>
          </div>
