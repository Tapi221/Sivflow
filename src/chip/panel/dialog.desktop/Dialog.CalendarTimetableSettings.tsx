      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-5 py-4">
        <div className="mb-5">
          ...
          <div className="flex gap-2">
            {VISIBLE_DAY_COUNT_OPTIONS.map((count) => {
              ...
              return (
                <button key={count} type="button" className={cn("rounded-full border px-3 py-1.5 text-xs font-semibold", visibleDayCount === visibleCount ? "border-[#007aff] bg-blue-50 text-blue-500" : "border-zinc-200 bg-white text-zinc-500")} onClick={() => {
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            <button type="button" className="rounded-full border border-zinc-200 px-3 py-1.5 text-xs font-bold text-blue-500" onClick={() => {
              void onAddPeriod(); }}
            >時限を追加</button>
          </div>
          {periods.map((period) => (
            <div key={period.id} className="grid grid-cols-[52px_minmax(0,1fr)_minmax(0,1fr)_auto] items-center gap-2 rounded-2xl border border-slate-200 bg-[#fafafa] p-2">
              <input className="h-9 rounded-xl border border-zinc-200 bg-white px-2 text-center text-xs font-bold text-zinc-900" value={period.label} onChange={(event) => {
                void onUpdatePeriod({ ...period, label: event.target.value }); }} aria-label="時限名"
              />
              <input className="h-9 rounded-xl border border-zinc-200 bg-white px-2 text-xs font-semibold text-zinc-900" type="time" value={period.startTime} onChange={(event) => {
                void onUpdatePeriod({ ...period, startTime: event.target.value }); }} aria-label={`${period.label}限の開始時刻`}
              />
              <input className="h-9 rounded-xl border border-zinc-200 bg-white px-2 text-xs font-semibold text-zinc-900" type="time" value={period.endTime} onChange={(event) => {
