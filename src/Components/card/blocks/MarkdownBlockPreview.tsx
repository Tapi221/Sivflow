...
/**
 * 互換エクスポート
 * - 既存コードは MarkdownBlockPreview(markdown=...) を期待している
 * - 実体は MarkdownBlockView(md=...) なのでラップして橋渡しする
 */
export const MarkdownBlockPreview: React.FC<MarkdownBlockPreviewProps> = ({
  markdown,
  ...rest
}) => {
  return <MarkdownBlockView md={markdown} {...rest} />;
};
...
export const MarkdownBlockView: React.FC<MarkdownBlockViewProps> = ({
  ...
  const components = useMemo<Components>(
    () => ({
      ...
      h1: ({ children }) => (
        <h1 className="m-0 font-serif text-[24px] font-medium leading-[48px]">
          {children}
        </h1>
      ),
      h2: ({ children }) => (
        <h2 className="m-0 font-serif text-[20px] font-medium leading-[24px]">
          {children}
        </h2>
      ),
      h3: ({ children }) => (
        <h3 className="m-0 font-serif text-[18px] font-medium leading-[24px]">
          {children}
        </h3>
      ),
      h4: ({ children }) => (
        <h4 className="m-0 font-serif text-[16px] font-medium leading-[24px]">
          {children}
        </h4>
      ),
