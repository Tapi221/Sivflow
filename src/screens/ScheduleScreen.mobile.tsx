...
export const ScheduleScreen = ({ initialActiveMode, onClose }: ScheduleScreenProps) => {
  ...
  return (
    <div className="flex h-full min-h-0 w-full flex-col overflow-hidden bg-[#f7f3eb] text-[#1c1c1e]">
      <header className="shrink-0 border-b border-[#ece4d8] bg-[rgba(247,243,235,0.94)] px-4 pb-3 pt-[calc(env(safe-area-inset-top)+14px)] backdrop-blur-xl">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            <p className="text-[12px] font-semibold uppercase tracking-[0.18em] text-[#8a8378]">Schedule</p>
          ...
          <div className="flex shrink-0 items-center gap-2">
            <button type="button" className="flex h-9 min-w-9 items-center justify-center rounded-full border border-[#e3dace] bg-white px-3 text-[13px] font-semibold text-[#2366c6] shadow-sm shadow-black/5" onClick={handleToday}>
              {t.todayButton}
            </button>
            {onClose ? (
              <button type="button" className="flex h-9 min-w-9 items-center justify-center rounded-full border border-[#e3dace] bg-white px-3 text-[13px] font-semibold text-[#5e5a52] shadow-sm shadow-black/5" onClick={onClose}>
                Close
              </button>
            ) : (
              <button type="button" className="flex h-9 w-9 items-center justify-center rounded-full border border-[#d9e7ff] bg-[#eef5ff] text-[20px] font-semibold leading-none text-[#2366c6] shadow-sm shadow-black/5" onClick={() => { void addGoogleCalendar(); }} aria-label={t.addGoogleCalendar}>
                +
              </button>
            )}
          </div>
        ...
        <div className="mt-4 grid grid-cols-3 rounded-[18px] bg-[#ebe4d8] p-1">
          {modeOptions.map((option) => (
            <button key={option.value} type="button" className={cn("h-9 rounded-[14px] text-[13px] font-semibold transition", activeMode === option.value ? "bg-white text-[#1c1c1e] shadow-sm shadow-black/10" : "text-[#777064]")} onClick={() => setActiveMode(option.value)} aria-pressed={activeMode === option.value}>
              {option.label}
            </button>
          ))}
        </div>
        ...
        <div className="mt-3 flex items-center justify-between gap-3">
          <div className="flex min-w-0 items-center gap-1 rounded-full bg-white p-1 shadow-sm shadow-black/5">
            {viewOptions.map((option) => (
              <button key={option.value} type="button" className={cn("h-8 rounded-full px-3 text-[12px] font-semibold transition", selectedViewMode === option.value ? "bg-[#1f6feb] text-white" : "text-[#706a60]")} onClick={() => handleSelectViewMode(option.value)} aria-pressed={selectedViewMode === option.value}>
                {option.label}
              </button>
            ))}
          </div>
          ...
          <div className="flex items-center gap-1">
            <button type="button" className="flex h-8 w-8 items-center justify-center rounded-full bg-white text-[18px] font-semibold text-[#706a60] shadow-sm shadow-black/5" onClick={handlePrevious} aria-label={t.previousLabel}>
              ‹
            </button>
            <button type="button" className="flex h-8 w-8 items-center justify-center rounded-full bg-white text-[18px] font-semibold text-[#706a60] shadow-sm shadow-black/5" onClick={handleNext} aria-label={t.nextLabel}>
              ›
            </button>
          </div>
        </div>
      ...
      <main className="min-h-0 flex-1 overflow-y-auto px-4 pb-[calc(env(safe-area-inset-bottom)+20px)] pt-4">
        {activeMode === "task" ? (
          <section className="min-h-[calc(100dvh-220px)] overflow-hidden rounded-[26px] border border-[#e9dfd2] bg-white shadow-sm shadow-black/5">
          ...
          <div className="flex flex-col gap-4">
            <section className="rounded-[26px] border border-[#e9dfd2] bg-white p-4 shadow-sm shadow-black/5">
              <div className="flex items-center justify-between gap-3">
                <div>
                  <p className="text-[12px] font-semibold text-[#8a8378]">{format(selectedDate, "M/d EEE", { locale: dateFnsLocale })}</p>
                  <h2 className="mt-1 text-[19px] font-semibold tracking-[-0.03em] text-[#1c1c1e]">
                    {activeMode === "timeline" ? t.timelineTab : t.calendarTab}
                  </h2>
                </div>
                <div className="rounded-full bg-[#f3f7ff] px-3 py-1 text-[12px] font-semibold text-[#2366c6]">
              ...
              <div className="mt-4 grid grid-cols-7 gap-1">
                {visibleWeekDays.map((day) => {
                  ...
                  return (
                    <button key={day.toISOString()} type="button" className={cn("flex min-h-[62px] flex-col items-center justify-center rounded-[18px] px-1 transition", isSelected ? "bg-[#1f6feb] text-white shadow-lg shadow-[#1f6feb]/25" : "bg-[#f8f5ef] text-[#4d473f]")} onClick={() => handleSelectDate(day)} aria-pressed={isSelected}>
                      <span className={cn("text-[10px] font-semibold", isSelected ? "text-white/75" : "text-[#8a8378]")}>{format(day, "EEE", { locale: dateFnsLocale })}</span>
            ...
            {activeMode === "calendar" && selectedViewMode === "month" ? (
              <section className="rounded-[26px] border border-[#e9dfd2] bg-white p-4 shadow-sm shadow-black/5">
                <div className="grid grid-cols-7 gap-1 text-center text-[11px] font-semibold text-[#9b9288]">
                ...
                <div className="mt-2 grid grid-cols-7 gap-1">
                  {monthGridDays.map((day) => {
                    ...
                    return (
                      <button key={day.toISOString()} type="button" className={cn("min-h-[48px] rounded-[15px] border text-left transition", isSelected ? "border-[#1f6feb] bg-[#eef5ff]" : "border-transparent bg-transparent", !isInMonth ? "opacity-35" : "opacity-100")} onClick={() => handleSelectDate(day)} aria-pressed={isSelected}>
                        <span className={cn("ml-2 mt-2 inline-flex h-6 min-w-6 items-center justify-center rounded-full px-1 text-[12px] font-semibold", isCurrentDate ? "bg-[#1f6feb] text-white" : "text-[#39342e]")}>{format(day, "d")}</span>
            ...
            <section className="rounded-[26px] border border-[#e9dfd2] bg-white p-4 shadow-sm shadow-black/5">
              <div className="flex items-center justify-between gap-3">
                ...
                <span className="rounded-full bg-[#f7f3eb] px-2.5 py-1 text-[12px] font-semibold text-[#8a8378]">{primaryEvents.length}</span>
              </div>

              <div className="mt-3 flex flex-col gap-2">
                {primaryEvents.length > 0 ? (
                  primaryEvents.map((event) => (
                    <article key={event.id} className="flex gap-3 rounded-[20px] border border-[#f0e8dc] bg-[#fbfaf7] p-3">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="mt-1 text-[12px] font-medium text-[#8a8378]">{getEventTimeLabel(event, t.allDay)}</p>
                        {event.location ? <p className="mt-1 truncate text-[12px] text-[#9b9288]">{event.location}</p> : null}
                      </div>
                    </article>
                  ))
                ) : (
                  <div className="rounded-[20px] border border-dashed border-[#e8ded1] bg-[#fbfaf7] px-4 py-8 text-center">
                    <p className="text-[14px] font-semibold text-[#6f675d]">予定はありません</p>
                    <p className="mt-1 text-[12px] text-[#9b9288]">Google Calendar やタスクを追加するとここに表示されます。</p>
                  </div>
                )}
              </div>
            </section>
