...
const saveImageToIndexedDb = async (image: UploadedImage): Promise<void> => { assertImageInvariant(image);
...
const getImageFromIndexedDb = async (imageId: string): Promise<UploadedImage | null> => { try { const db = await getLocalDb();
