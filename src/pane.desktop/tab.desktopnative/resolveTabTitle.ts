...
export const resolveCardSetTabTitle = (cardSet: CardSet): string => { const name = cardSet.name?.trim();
  return name || "無題のセット";
};

export const resolveCardTabTitle = (card: Card): string => { const title = card.title?.trim();
  if (title) return title;
