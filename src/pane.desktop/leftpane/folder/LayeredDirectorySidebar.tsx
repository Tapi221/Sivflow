...
import type { CardSetContextMenuAction } from "@/chip/rightclickpanel.desktop/CardSetContextMenu.desktop";
import { CARD_SET_CONTEXT_MENU_HEIGHT, CARD_SET_CONTEXT_MENU_PANEL_ID, CARD_SET_CONTEXT_MENU_WIDTH, CardSetContextMenu } from "@/chip/rightclickpanel.desktop/CardSetContextMenu.desktop";
import type { DocumentContextMenuAction } from "@/chip/rightclickpanel.desktop/DocumentContextMenu.desktop";
import { DOCUMENT_CONTEXT_MENU_HEIGHT, DOCUMENT_CONTEXT_MENU_PANEL_ID, DOCUMENT_CONTEXT_MENU_WIDTH, DocumentContextMenu } from "@/chip/rightclickpanel.desktop/DocumentContextMenu.desktop";
...
import type { LayeredProjectMenuAction, LayeredProjectMenuActionId, LayeredProjectMenuSubmenuAnchor } from "@/chip/rightclickpanel.desktop/LayeredProjectMenu";
import { LAYERED_PROJECT_MENU_HEIGHT, LAYERED_PROJECT_MENU_PANEL_ID, LAYERED_PROJECT_MENU_WIDTH, LayeredProjectMenu } from "@/chip/rightclickpanel.desktop/LayeredProjectMenu";
...
const useFolderContextMenu = ({ createFolder, updateFolder, deleteFolder, createCardSet, createNote, openNoteTab, getNextOrderIndex, setExpandedFolderIds }: UseFolderContextMenuParams) => {
...
const DirectoryEntityRow = ({ id, label, kind, level, isSelected, onSelect, onContextMenu }: DirectoryEntityRowProps) => {
  ...
  return (
    <div role="treeitem" tabIndex={0} aria-level={level} aria-selected={isSelected} data-directory-entity-id={id} data-directory-entity-kind={kind} onClick={handleClick} onKeyDown={handleKeyDown} onContextMenu={handleContextMenu} className={cn("group/directory-tree-row relative flex h-8 cursor-default items-center gap-2 rounded-[8px] pr-2 text-[14px] font-medium text-[var(--app-sidebar-text)] transition-[background,box-shadow,opacity,transform] duration-150 hover:bg-[#eee] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#c7c7c7]", isSelected && "bg-[#e9e9e9]")} style={{ paddingLeft: rowPaddingLeft }}>
...
const ProjectListSidebar = ({ onOpenCardSet }: ProjectListSidebarProps) => {
  ...
  const { contextMenuElement: folderContextMenuElement, openContextMenu: openFolderContextMenu } = useFolderContextMenu({ createFolder, updateFolder, deleteFolder, createCardSet, createNote, openNoteTab, getNextOrderIndex, setExpandedFolderIds });
