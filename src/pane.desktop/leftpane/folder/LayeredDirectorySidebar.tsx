...
const IconFolder = ({ className }: { className?: string }) => <svg viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg" className={cn("h-[18px] w-[18px] shrink-0", className)}><path d="M3 6.4A2.15 2.15 0 0 1 5.15 4.25h2.08c.43 0 .84.17 1.15.47l.8.78h5.67A2.15 2.15 0 0 1 17 7.65v5.6a2.5 2.5 0 0 1-2.5 2.5h-9A2.5 2.5 0 0 1 3 13.25V6.4Z" stroke="currentColor" strokeWidth="1.35" strokeLinejoin="round" /></svg>;
const IconProject = ({ className }: { className?: string }) => <svg viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg" className={cn("h-[18px] w-[18px] shrink-0", className)}><path d="M7.25 6V5.1A1.85 1.85 0 0 1 9.1 3.25h1.8a1.85 1.85 0 0 1 1.85 1.85V6" stroke="currentColor" strokeWidth="1.35" strokeLinecap="round" strokeLinejoin="round" /><path d="M4.75 6.25h10.5A2.25 2.25 0 0 1 17.5 8.5v5.25A2.25 2.25 0 0 1 15.25 16H4.75a2.25 2.25 0 0 1-2.25-2.25V8.5a2.25 2.25 0 0 1 2.25-2.25Z" stroke="currentColor" strokeWidth="1.35" strokeLinejoin="round" /><path d="M2.75 10.25h14.5" stroke="currentColor" strokeWidth="1.35" strokeLinecap="round" /><path d="M8.9 10.25h2.2v1.45H8.9v-1.45Z" stroke="currentColor" strokeWidth="1.35" strokeLinejoin="round" /></svg>;
...
const ProjectListItem = ({ folder, selectedFolderId, onSelectProject, onOpenContextMenu }: ProjectListItemProps) => {
  ...
  return <div data-folder-id={folderId}><div role="treeitem" aria-level={ROOT_LEVEL} aria-selected={isSelected} onContextMenu={handleContextMenu} className={cn("flex h-8 items-center rounded-[8px] pl-1.5 pr-2 text-[14px] font-medium text-[var(--app-sidebar-text)]", isSelected && "bg-[#e9e9e9]")}><button type="button" onClick={handleRowClick} title={folderName} className="flex h-8 min-w-0 flex-1 items-center gap-2 rounded-[8px] text-left text-inherit hover:bg-[#eeeeee]"><IconProject className="layered-directory-row-icon" /><span className="min-w-0 flex-1 truncate">{folderName}</span></button></div></div>;
};
