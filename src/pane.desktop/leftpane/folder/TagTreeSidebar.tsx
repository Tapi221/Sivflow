...
const getTagParentId = (tag: TagRecord): string | null => {
  return typeof tag.parentId === "string" && tag.parentId.trim().length > 0 ? tag.parentId : null;
};
...
const getRootTagIds = (nodes: TagTreeNode[]): string[] => {
  return nodes.map((node) => node.id);
};

const flattenVisibleTagTree = (nodes: TagTreeNode[], expandedTagIds: Set<string>, level = ROOT_LEVEL, visitedTagIds = new Set<string>()): VisibleTagTreeNode[] => {
...
const TagTreeSidebar = () => {
  ...
  const handleSelectTag = useCallback((tagName: string) => {
    if (tagFilter.length === 1 && tagFilter[0] === tagName) {
      clearTagFilter();
    } else {
  ...
  return (
    <aside aria-label="Tag tree explorer" className="h-full min-h-0 overflow-hidden">
      <div className="h-full min-h-0 overflow-y-auto px-2 pb-2 pt-1">
        <div role="tree" aria-label="タグツリー" className="flex flex-col gap-0.5">
          {visibleTagItems.length > 0 ? visibleTagItems.map((item) => (
            <TagTreeRow key={item.id} item={item} selectedTagNames={selectedTagNames} tagContentCountById={tagContentCountById} onToggleTag={handleToggleTag} onSelectTag={handleSelectTag} />
          )) : <p className="px-2 py-2 text-[12px] font-medium text-[#9aa1ad]">タグがありません</p>}
        </div>
      </div>
    </aside>
  );
