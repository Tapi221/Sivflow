...
const getSegmentLabel = (event: GoogleCalendarEvent, appProjects: AppCalendarItem[], calendarLabels: Map<string, string>): string => {
...
const buildDays = (dates: Date[], events: GoogleCalendarEvent[], selectedDate: Date, appProjects: AppCalendarItem[], googleAccounts: GoogleAccountDisplay[]): SplitDay[] => {
  ...
  dates.forEach((date) => { const key = getCalendarDateKey(date); eventsByDay.set(key, []); dayByKey.set(key, date); });
...
const SplitDaySection = memo(({ day, onSelectDate }: { day: SplitDay; onSelectDate?: (date: Date) => void }) => {
...
const CalendarListPieChartSplitViewComponent = ({ virtualRail, selectedDate, events, appProjects, googleAccounts, onSelectDate, onVisibleMonthChange, onVisibleDateChange, className }: CalendarListPieChartSplitViewProps) => {
  ...
  const updateVisibleDate = useCallback((element: HTMLDivElement | null) => { if (!element) return; const index = Math.max(0, Math.min(rail.totalDayCount - 1, Math.floor((element.scrollTop + Math.min(ANCHOR_OFFSET, element.clientHeight / 2)) / DAY_BLOCK))); const date = getScheduleVirtualRailDate(rail, index); if (!date) return; const key = getCalendarDateKey(date); if (lastVisibleKeyRef.current === key) return; lastVisibleKeyRef.current = key; onVisibleMonthChange?.(date); onVisibleDateChange?.(date); }, [onVisibleDateChange, onVisibleMonthChange, rail]);
  const scheduleScrollWork = useCallback((element: HTMLDivElement) => { pendingRef.current = element; if (frameRef.current !== null) return; frameRef.current = window.requestAnimationFrame(() => { frameRef.current = null; const pending = pendingRef.current; pendingRef.current = null; if (!pending) return; updateRange(pending); updateVisibleDate(pending); }); }, [updateRange, updateVisibleDate]);

  useLayoutEffect(() => { updateRange(scrollRef.current, true); }, [updateRange]);
  useEffect(() => { const element = scrollRef.current; if (!element) return; const handleScroll = () => scheduleScrollWork(element); element.addEventListener("scroll", handleScroll, { passive: true }); return () => element.removeEventListener("scroll", handleScroll); }, [scheduleScrollWork]);
  useEffect(() => { const element = scrollRef.current; const key = getCalendarDateKey(selectedDate); const index = getIndexForDate(rail, selectedDate); if (lastSelectedKeyRef.current === key || !element || index < 0 || index >= rail.totalDayCount) return; lastSelectedKeyRef.current = key; element.scrollTop = Math.max(0, index * DAY_BLOCK - SELECTED_OFFSET); updateRange(element, true); }, [rail, selectedDate, updateRange]);
  useEffect(() => () => { if (frameRef.current !== null) window.cancelAnimationFrame(frameRef.current); }, []);

  return <div className={cn("ml-4 mr-4 flex min-h-0 flex-1 overflow-hidden bg-white", className)}><div ref={scrollRef} className="min-h-0 flex-1 overflow-y-auto pb-6 pt-2 scrollbar-hidden"><div className="relative min-w-0" style={{ height: totalHeight }}>{days.map((day, offset) => <div key={day.key} className="absolute left-0 right-0" style={{ contain: "layout style", top: (range.start + offset) * DAY_BLOCK, height: DAY_HEIGHT }}><SplitDaySection day={day} onSelectDate={onSelectDate} /></div>)}</div></div></div>;
};
