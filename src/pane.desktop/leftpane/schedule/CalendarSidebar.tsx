    ...
    <div className="mt-0.5 flex flex-col gap-0.5">
      ...
      {isAdding && (
        <div className="mx-2 ml-2 mt-1 flex flex-col gap-1">
          ...
          {addError && (
            <p id="app-project-add-error" className="px-3 text-[10px] font-semibold text-[#c25f5f]">{addError}</p>
          )}
        </div>
      )}
    </div>
  ...
  return (
    <div className="mt-2">
      <button type="button" className="group flex h-7 w-full items-center gap-1.5 rounded-[10px] px-1.5 text-left transition-all duration-150 hover:bg-[#f7f7f7] active:bg-[#f1f1f1]" onClick={() => setIsOpen((value) => !value)} aria-expanded={isOpen}>
        ...
        <span className="truncate text-[11px] font-semibold tracking-wider text-[#9a9a9a]">{accountName}</span>
  ...
  return (
    <div className="mt-2">
      <div className="mb-1 flex h-6 shrink-0 items-center gap-1.5 px-2">
        <GoogleIcon className="h-3.5 w-3.5 text-[#9a9a9a]" label={GOOGLE_CALENDARS_LABEL} />
        <span className="min-w-0 flex-1 truncate text-[11px] font-bold uppercase tracking-[0.04em] text-[#9a9a9a]">{GOOGLE_CALENDARS_LABEL}</span>
...
const CalendarSidebar = ({ monthDate, selectedDate, visibleEvents, appProjects, projectCalendarLinks, googleCalendarColorOverrides, googleAccounts, isAnyCalendarConnecting, onSelectDate, onPreviousMonth, onNextMonth, onAddCalendar, onAddProject, onToggleProject, onLinkGoogleCalendarAsProject, onLinkProjectToGoogleCalendar, onCreateProjectGoogleCalendar, onUnlinkProjectCalendar, onChangeGoogleCalendarColor, onReconnectAccount, onToggleCalendar }: CalendarSidebarProps) => {
  ...
  return (
    <aside className="flex h-full min-h-0 w-[220px] shrink-0 flex-col overflow-hidden bg-transparent pb-0 pl-0 pr-3 pt-2 text-[#2f2f2f]">
      ...
      <nav className="mt-2 flex min-h-0 w-full flex-1 flex-col overflow-hidden pb-0" aria-label="カレンダー一覧">
        <div className="mb-1 flex h-6 shrink-0 items-center gap-1.5 px-2">
          <button type="button" className="group flex min-w-0 flex-1 items-center gap-1.5 rounded-[10px] text-left transition-all duration-150 hover:text-[#5f6574]" onClick={handleToggleAppProjectsOpen} aria-expanded={isAppProjectsOpen}>
            <span className={cn("flex h-3.5 w-3.5 shrink-0 items-center justify-center rounded-full text-[#b3b3b3] transition-all duration-200 group-hover:bg-white group-hover:text-[#8c8c8c]", !isAppProjectsOpen && "-rotate-90")}>
              <IconChevronRight className="h-3 w-3" />
            </span>
            <CalendarIcon className="h-3.5 w-3.5 shrink-0 text-[#9a9a9a]" />
            <span className="min-w-0 flex-1 truncate text-[11px] font-bold uppercase tracking-[0.04em] text-[#9a9a9a]">{t.myProjects}</span>
          </button>
        ...
        <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
          <div className="max-h-[55%] shrink-0 overflow-y-auto pb-1">
            {isAppProjectsOpen && <AppProjectsSection projects={appProjects} isAdding={isAddingProject} onAddProject={onAddProject} onToggleProject={onToggleProject} onOpenProjectLinksContextMenu={handleOpenProjectLinksContextMenu} onAddingChange={setIsAddingProject} />}
          </div>

          <div className="min-h-0 flex-1 overflow-y-auto border-t border-[#eeeeee] pt-1">
