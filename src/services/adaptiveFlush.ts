...
export class AdaptiveFlushService {
  ...
  getNetworkDiagnostics(): {
    ...
    if (avgRtt > 500) {
      recommendation = 'Poor network detected. Using large batches to reduce round trips.';
      // ネットワークが悪いので、回数を減らすためにバッチを大きくしている
    } else if (avgRtt < 100) {
      recommendation = 'Excellent network detected. Using small batches for low latency.';
