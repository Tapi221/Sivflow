...
const openImageFileDb = async (): Promise<IDBDatabase> =>
  new Promise((resolve, reject) => {
    ...
    request.onerror = () => reject(request.error ?? new Error('Failed to open image file store'));
  });

const getStoredImageFile = async (id: string): Promise<StoredImageFile | null> => {
...
const putScopedImageBlob = async (id: string, blob: Blob): Promise<void> => {
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    tx.onerror = () => reject(tx.error ?? new Error('Failed to save image blob'));
...
export const getImageBlob = async (id: string, options?: BlobScopeOptions): Promise<Blob | null> => {
...
export const deleteImageBlob = async (id: string, options?: BlobScopeOptions): Promise<void> => {
  const db = await openImageFileDb();
  const scopedId = makeScopedId(id, options);
  await new Promise<void>((resolve, reject) => {
    const tx = db.transaction(STORE_NAME, 'readwrite');
    ...
    tx.onabort = () => reject(tx.error ?? new Error('Image blob delete aborted'));
  });
};
...
export const deleteImageBlobsByUser = async (userId: string): Promise<void> => {
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    tx.oncomplete = () => resolve();
    tx.onerror = () => reject(tx.error ?? new Error('Failed to delete image blobs by user'));
    tx.onabort = () => reject(tx.error ?? new Error('Image blob cleanup aborted'));
    request.onerror = () => reject(request.error ?? new Error('Failed to scan image blobs'));
  });
};
