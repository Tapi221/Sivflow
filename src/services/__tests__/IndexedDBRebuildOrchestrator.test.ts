...
describe('IndexedDBRebuildOrchestrator', () => {
  ...
  it('continues rebuild with degraded success when one item fails', async () => {
    ...
    pullDiffMock.mockResolvedValue({
      changes: [
        { type: 'card', id: 'ok-card', data: { id: 'ok-card', questionBlocks: [] } },
        { type: 'card', id: 'bad-card', data: { id: 'bad-card', questionBlocks: [] } },
      ],
    });
    getLocalDbMock.mockResolvedValueOnce(oldDb as any).mockResolvedValueOnce(newDb as any);

    const { IndexedDBRebuildOrchestrator } = await import('../IndexedDBRebuildOrchestrator');
    ...
    expect(upsert).toHaveBeenCalledWith('cards', expect.objectContaining({ id: 'ok-card' }), true);
