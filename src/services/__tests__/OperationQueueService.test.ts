...
describe('OperationQueueService', () => {
    ...
    describe('Priority Resolution', () => {
        it('should upgrade priority to high', async () => {
            ...
            await operationQueue.enqueueChange('card', targetId, 'update', { v: 1 }, 'background');
            ...
            await operationQueue.enqueueChange('card', targetId, 'update', { v: 2 }, 'normal');
            const itemNormal = await db.syncQueue.where({ targetId }).first();
            expect(itemNormal?.priority).toBe('normal');

            // Compress with High -> High
            await operationQueue.enqueueChange('card', targetId, 'update', { v: 3 }, 'high');
            ...
            await operationQueue.enqueueChange('card', targetId, 'update', { v: 4 }, 'background');
    ...
    describe('Processing & Retry', () => {
        it('should retry on failure and eventually move to DLQ', async () => {
            ...
            const processorSpy = vi.spyOn(operationQueue as any, 'performSyncOperation')
            ...
            for (let i = 0; i <= 5; i++) {
                ...
                await db.syncQueue.where({ targetId }).modify({ nextRetryAt: 0, status: 'pending' });
