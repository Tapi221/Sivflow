...
const getEntityId = (obj: unknown): string | undefined => {
  if (typeof obj !== 'object' || obj === null || !('id' in obj)) return undefined;
...
export const attachHooks = (db: LocalDB): void => {
  ...
  cardsTable.hook('creating', (_primaryKey, obj) => {
    assertNoBlobUrlInCardPayload(obj, { entityType: 'card', entityId: getEntityId(obj) });
  });
  cardsTable.hook('updating', (mods, _primaryKey, obj) => {
    const candidate = buildCardCandidateFromMods(obj, mods);
    assertNoBlobUrlInCardPayload(candidate, { entityType: 'card', entityId: getEntityId(obj) });
  });
};
