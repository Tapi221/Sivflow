           ...
           if (cursor) {
             ...
             try {
               ...
               if ((isFolder || isCard) && id && id !== '0,0') {
                 let data = flattenFirestoreDocument(record.document?.data || record.data || record.value || record);
...
export async function importFromDatabase(
  ...
  if (sourceDbName.includes('/main') || sourceDbName.includes('firestore')) {
      const result = await extractFromFirestoreSDK(db, sourceDbName, currentUserId, onProgress);
