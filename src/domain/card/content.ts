...
const getCardBlocks = (card: Card, side: CardSide) => { return getCardFace(card, side)?.blocks ?? EMPTY_BLOCKS;
};
const getCardInk = (card: Card, side: CardSide) => { return getCardFace(card, side)?.ink ?? null;
};
const getCardExtraRows = (card: Card, side: CardSide) => { return getCardFace(card, side)?.extraRows ?? 0;
};
const extractCardTextFromBlocks = (blocks: CardBlock[]) => { for (const block of blocks) { if (block.type === "text" && typeof block.content === "string") { const content = block.content.trim();
...
const getCardText = (card: Card, side: CardSide) => { return extractCardTextFromBlocks(getCardBlocks(card, side));
};
const getCardImages = (card: Card, side: CardSide) => { const images = getCardBlocks(card, side).filter((block): block is CardBlock & { type: "image"; images: UploadedImage[]; } =>
  block.type === "image" && Array.isArray(block.images),
)
  .flatMap((block) => block.images);
return images.length > 0 ? images : EMPTY_IMAGES;
};
const getCardAudios = (card: Card, side: CardSide) => { const audios = getCardBlocks(card, side).filter((block): block is CardBlock & { type: "audio";
...
const getCardCode = (card: Card, side: CardSide) => { const codeBlock = getCardBlocks(card, side).find((block): block is CardBlock & { type: "code"; code: CodeBlockData; } =>
