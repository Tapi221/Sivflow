...
export const buildCardSetById = <TCardSet extends CardSetLike>(cardSets: readonly TCardSet[]) => {
...
export const resolveCardFolderIdStrict = (card: CardLike, cardSetById: ReadonlyMap<string, CardSetLike>) => {
  const cardSetId = getCardLikeCardSetId(card);
...
export const didUseLegacyFolderFallback = (card: CardLike, cardSetById: ReadonlyMap<string, CardSetLike>) => {
  const cardSetId = getCardLikeCardSetId(card);
...
export const resolveCardFolderId = (card: CardLike, cardSetById: ReadonlyMap<string, CardSetLike>) => {
...
export const isCardInFolder = (card: CardLike, folderId: string | null | undefined, cardSetById: ReadonlyMap<string, CardSetLike>) => {
  const targetFolderId = normalizeFolderId(folderId);
  return resolveCardFolderIdStrict(card, cardSetById) === targetFolderId;
};
export const filterCardsByFolderId = <T extends CardLike>(cards: readonly T[], folderId: string | null | undefined, cardSetById: ReadonlyMap<string, CardSetLike>) => {
  const targetFolderId = normalizeFolderId(folderId);
  return cards.filter(
