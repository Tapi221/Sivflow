...
const isGridOffsetType = (type: CardBlock["type"]) => { return GRID_OFFSET_TYPES.has(type);
};
const getNormalizedGridOffsetRows = (block: CardBlock): number => { if (!isGridOffsetType(block.type)) return 0;
