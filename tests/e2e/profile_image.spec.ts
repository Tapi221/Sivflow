...
test.describe("Profile Image UI", () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.goto(
      "http://localhost:5173/folders?test_bypass=true&settings=true&settingsTab=account",
  ...
  test("Should display deterministic avatar color for user without image", async ({
    page,
  }) => {
  ...
  test("Should handle broken image URL by falling back to avatar", async ({
    page,
  }) => {
    await expect(
