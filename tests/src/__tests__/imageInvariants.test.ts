...
describe('UploadedImage 不変条件テスト', () => {
  ...
  it('remoteUrl に Base64 (data:) が含まれてはならない', () => {
    ...
    expect(() => strictValidateBeforeSave(invalidImage)).toThrow('Base64 in remoteUrl');
  });
  ...
  it('Base64 の MIME 型チェック', () => {
    ...
    expect(() => strictValidateBeforeSave(invalidImage)).toThrow('Base64 encoding detected');
  });
  ...
  it('localUrl に Base64 (data:) が含まれてはならない', () => {
    ...
    expect(() => strictValidateBeforeSave(invalidImage)).toThrow('Base64 in localUrl');
  });
