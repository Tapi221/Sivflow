import { addContextCategoryMutation, addContextDocMutation, addContextFileMutation, ContextCategories as GraphQLContextCategories, createCopilotContextMutation, createCopilotMessageMutation, createCopilotSessionMutation, forkCopilotSessionMutation, getCopilotSessionQuery, getTranscriptTaskQuery, listContextObjectQuery, listContextQuery, matchFilesQuery, matchWorkspaceDocsQuery, removeContextDocMutation, removeContextFileMutation, settleTranscriptTaskMutation, submitTranscriptTaskMutation, updateCopilotSessionMutation, } from '@affine/graphql';
import { ContextCategories } from '../../models';
import { TestingApp } from './testing-app';

export const cleanObject = ( obj: any[] | undefined, condition = ['id', 'status', 'error', 'sessionId', 'createdAt'] ) => JSON.parse( JSON.stringify(obj || [], (k, v) => condition.includes(k) || v === null ? undefined : v ) );

export const createCopilotSession = async (app: TestingApp, workspaceId: string, docId: string | null, promptName: string, pinned: boolean = false): Promise<string> => { const res = await app.gql({ query: createCopilotSessionMutation, variables: { options: { workspaceId, docId, promptName, pinned } }, });

  return res.createCopilotSession;
};

export const createWorkspaceCopilotSession = async (app: TestingApp, workspaceId: string, promptName: string): Promise<string> => { return createCopilotSession(app, workspaceId, null, promptName);
};

export const createPinnedCopilotSession = async (app: TestingApp, workspaceId: string, docId: string, promptName: string): Promise<string> => { return createCopilotSession(app, workspaceId, docId, promptName, true);
};

export const createDocCopilotSession = async (app: TestingApp, workspaceId: string, docId: string, promptName: string): Promise<string> => { return createCopilotSession(app, workspaceId, docId, promptName);
};

export const getCopilotSession = async (app: TestingApp, workspaceId: string, sessionId: string): Promise<{ id: string;
  docId: string | null;
...
export const updateCopilotSession = async (app: TestingApp, sessionId: string, promptName: string): Promise<string> => { const res = await app.gql({ query: updateCopilotSessionMutation, variables: { options: { sessionId, promptName } }, });

  return res.updateCopilotSession;
};

export const forkCopilotSession = async (app: TestingApp, workspaceId: string, docId: string, sessionId: string, latestMessageId?: string): Promise<string> => { const res = await app.gql({ query: forkCopilotSessionMutation, variables: { options: { workspaceId, docId, sessionId, latestMessageId } }, });

  return res.forkCopilotSession;
};

export const createCopilotContext = async (app: TestingApp, workspaceId: string, sessionId: string): Promise<string> => { const res = await app.gql({ query: createCopilotContextMutation, variables: { workspaceId, sessionId }, });

  return res.createCopilotContext;
};

export const matchFiles = async (app: TestingApp, contextId: string, content: string, limit: number): Promise< | { fileId: string;
      chunk: number;
      content: string;
...
export const matchWorkspaceDocs = async (app: TestingApp, contextId: string, content: string, limit: number): Promise< | { docId: string;
      chunk: number;
      content: string;
      distance: number | null;
    }[]
...
export const listContext = async (app: TestingApp, workspaceId: string, sessionId: string): Promise< { id: string;
    workspaceId: string;
  }[]
> => {
  const res = await app.gql({
...
export const addContextFile = async (app: TestingApp, contextId: string, fileName: string, content: Buffer): Promise<{ id: string }> => { const res = await app.gql({ query: addContextFileMutation, variables: { content: new File([content], fileName, { type: 'application/octet-stream', }), options: { contextId }, }, });

  return res.addContextFile;
};

export const removeContextFile = async (app: TestingApp, contextId: string, fileId: string): Promise<boolean> => { const res = await app.gql({ query: removeContextFileMutation, variables: { options: { contextId, fileId } }, });

  return res.removeContextFile;
};

export const addContextDoc = async (app: TestingApp, contextId: string, docId: string): Promise<{ id: string }[]> => { const res = await app.gql({ query: addContextDocMutation, variables: { options: { contextId, docId } }, });

  return [res.addContextDoc];
};

export const addContextCategory = async (app: TestingApp, contextId: string, type: ContextCategories, categoryId: string, docs: string[]): Promise<{ type: string; id: string; docs: { id: string }[] }> => {
  const graphqlType =
...
export const removeContextDoc = async (app: TestingApp, contextId: string, docId: string): Promise<boolean> => { const res = await app.gql({ query: removeContextDocMutation, variables: { options: { contextId, docId } }, });

  return res.removeContextDoc;
};

export const listContextDocAndFiles = async (app: TestingApp, workspaceId: string, sessionId: string, contextId: string): Promise< | { docs: { id: string;
        status: string | null;
        createdAt: number;
      }[];
      files: {
...
export const listContextCategories = async (app: TestingApp, workspaceId: string, sessionId: string, contextId: string): Promise< | { collections: { type: string;
        id: string;
        docs: {
          id: string;
          status: string | null;
...
export const submitTranscriptTask = async (app: TestingApp, workspaceId: string, blobId: string, fileName: string, content: Buffer[], input?: Record<string, unknown>): Promise<{ id: string; status: string }> => {
  const res = await app.gql({
...
export const settleTranscriptTask = async (app: TestingApp, workspaceId: string, taskId: string): Promise<{ id: string;
  status: string;
  title: string | null;
  summary: string | null;
  actions: string | null;
...
export const getTranscriptTask = async (app: TestingApp, workspaceId: string, taskId: string): Promise<{ id: string;
  status: string;
  title: string | null;
  summary: string | null;
  sourceAudio: {
...
export const createCopilotMessage = async (app: TestingApp, sessionId: string, content?: string, attachments?: string[], blob?: File, blobs?: File[], params?: Record<string, string>): Promise<string> => { const gql = { query: createCopilotMessageMutation.query, variables: { options: { sessionId, content, attachments, blob: null, blobs: [], params, }, }, };
...
export const chatWithText = async (app: TestingApp, sessionId: string, messageId?: string, prefix = '', retry?: boolean): Promise<string> => { const endpoint = prefix || '/stream';
...
export const chatWithTextStream = async (app: TestingApp, sessionId: string, messageId?: string) => { return chatWithText(app, sessionId, messageId, '/stream');
};

export const chatWithActionStream = async (app: TestingApp, sessionId: string, input: { actionId: string;
...
export const chatWithImages = async (app: TestingApp, sessionId: string, messageId?: string) => { return chatWithText(app, sessionId, messageId, '/images');
};

export const chatWithStreamObject = async (app: TestingApp, sessionId: string, messageId?: string) => { return chatWithText(app, sessionId, messageId, '/stream-object');
};

export const unsplashSearch = async (app: TestingApp, params: Record<string, string> = {}) => { const query = new URLSearchParams(params);
  const res = await app.GET(`/api/copilot/unsplash/photos?${query}`);
  return res;
};

export const sse2array = (eventSource: string) => { const blocks = eventSource.replace(/^\n(.*?)\n$/, '$1').split(/\n\n+/);
...
export const array2sse = (blocks: Record<string, string>[]) => { return blocks .map( e => '\n' + Object.entries(e) .filter(([k]) => !!k) .map(([k, v]) => `${k}: ${v}`) .join('\n') ) .join('\n');
};

export const textToEventStream = (content: string | string[], id: string, event = 'message'): string => { return ( Array.from(content) .map(x => `\nevent: ${event}\nid: ${id}\ndata: ${x}`) .join('\n') + '\n\n' );
};
...
export const getHistories = async (app: TestingApp, variables: { workspaceId: string;
    docId?: string | null;
...
export const getWorkspaceSessions = async (app: TestingApp, variables: { workspaceId: string;
    options?: HistoryOptions;
  }): Promise<History[]> => {
  const res = await app.gql(
...
export const getDocSessions = async (app: TestingApp, variables: { workspaceId: string;
    docId: string;
    options?: HistoryOptions;
  }): Promise<History[]> => {
  const res = await app.gql(
...
export const getPinnedSessions = async (app: TestingApp, variables: { workspaceId: string;
    docId?: string;
    messageOrder?: 'asc' | 'desc';
    withPrompt?: boolean;
  }): Promise<History[]> => {
