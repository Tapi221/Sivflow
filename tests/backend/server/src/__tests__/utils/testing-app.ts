...
export class TestingApp extends ApplyType<INestApplication>() {
  ...
  getCookies(): Record<string, string> {
    ...
    if (this.sessionCookie) {
      cookies[AuthService.sessionCookieName] = this.sessionCookie;
    }
    if (this.currentUserCookie) {
      cookies[AuthService.userCookieName] = this.currentUserCookie;
    }
    if (this.csrfCookie) {
      cookies[AuthService.csrfCookieName] = this.csrfCookie;
    }
    return cookies;
  }
  ...
  async login(user: MockedUser) {
    const authService = this.get(AuthService, { strict: false });
    const session = await authService.createUserSession(user.id);
    this.sessionCookie = session.id;
    this.currentUserCookie = user.id;
    this.userCookies.add(user.id);
  }
