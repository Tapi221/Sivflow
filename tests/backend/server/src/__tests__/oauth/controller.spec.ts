...
test('should be able to sign up with oauth', async t => {
  ...
  t.is(user!.authAccounts[0].providerAccountId, '1');
});
...
test('github oauth should resolve private email from emails api', async t => {
  ...
  t.truthy(user);
  t.is(user!.authAccounts[0].provider, OAuthProviderName.GitHub);
  t.is(user!.authAccounts[0].providerAccountId, 'github-user');
});
