...
describe('MarkdownBlockContent', () => {
  it('段落間の空行は24pxになる', () => {
    const { container } = render(<MarkdownBlockContent markdown={'first\n\nsecond'} />);

    const root = container.querySelector('.markdownBlockCardView') as HTMLElement | null;
  ...
  it('異なるブロック種別間の空行も24pxになる', () => {
    const { container } = render(<MarkdownBlockContent markdown={'paragraph\n\n- item'} />);
