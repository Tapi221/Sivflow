...
import { isBlobUrl, isStorageUrl, isBase64DataUrl, createBlobUrl, createStorageUrl } from '@/types/branded';
...
describe('Branded Types', () => {
  ...
  describe('isStorageUrl', () => {
    it('should return true for valid Firebase Storage URL', () => {
      expect(isStorageUrl('https://firebasestorage.googleapis.com/v0/b/bucket/o/image.jpg')).toBe(true);
      expect(isStorageUrl('https://storage.googleapis.com/bucket/image.jpg')).toBe(true);
    });
  ...
  describe('createBlobUrl', () => {
    ...
    it('should throw for invalid Blob URL', () => {
      expect(() => createBlobUrl('https://example.com/image.jpg')).toThrow(/Invalid BlobUrl/);
      expect(() => createBlobUrl('data:image/png;base64,abc')).toThrow(/Invalid BlobUrl/);
    });
  });
  ...
  describe('createStorageUrl', () => {
    ...
    it('should throw for invalid Storage URL', () => {
      expect(() => createStorageUrl('http://example.com/image.jpg')).toThrow(/Invalid StorageUrl/);
      expect(() => createStorageUrl('blob:http://localhost:5173/abc')).toThrow(/Invalid StorageUrl/);
    });
  });
});
