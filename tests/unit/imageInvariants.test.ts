...
describe('Image Invariants', () => {
  describe('assertNoBase64InImage', () => {
    it('should reject Base64 in remoteUrl', () => {
      ...
      expect(() => assertNoBase64InImage(image)).toThrow(ImageInvariantViolation);
      expect(() => assertNoBase64InImage(image)).toThrow(/Base64 detected in remoteUrl/);
    });
    ...
    it('should reject Base64 in localUrl', () => {
      ...
      expect(() => assertNoBase64InImage(image)).toThrow(ImageInvariantViolation);
      expect(() => assertNoBase64InImage(image)).toThrow(/Base64 detected in localUrl/);
    });
    ...
    it('should accept valid Storage URL in remoteUrl', () => {
      const image: UploadedImage = {
        id: 'test-4',
        remoteUrl: 'https://firebasestorage.googleapis.com/v0/b/bucket/o/image.jpg' as any,
        status: 'ready'
      };
  ...
  describe('assertImageInvariant', () => {
    ...
    it('should reject invalid remoteUrl (not HTTPS)', () => {
      ...
      expect(() => assertImageInvariant(image)).toThrow(ImageInvariantViolation);
    });
