...
describe('theme service', () => {
  ...
  test('generates a color property', () => {
    ...
    expect(themeService.generateColorProperty('--sivflow-hover-color')).toBe(
      'var(--sivflow-hover-color)'
    );

    expect(themeService.generateColorProperty('--sivflow-transparent')).toBe(
      'transparent'
    );
  ...
  test('gets a color value', () => {
    ...
    expect(themeService.getColorValue('--sivflow-transparent')).toBe(
      '--sivflow-transparent'
    );
    expect(
      themeService.getColorValue('--sivflow-transparent', 'transparent', true)
    ).toBe('transparent');
    expect(
      themeService.getColorValue('--sivflow-hover-color', 'transparent', true)
    ).toBe('rgba(0, 0, 0, 0.04)');
    expect(
      themeService.getColorValue('--sivflow-tooltip', undefined, true)
    ).toBe('rgba(0, 0, 0, 1)');
