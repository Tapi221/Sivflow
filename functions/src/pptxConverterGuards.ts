...
export const CONVERTER_PLACEHOLDER_ENABLED_ENV = "PPTX_CONVERTER_PLACEHOLDER_ENABLED";
...
export const buildDocumentPathPrefix = (userId: string, docId: string): string =>
  `users/${userId}/documents/${docId}/`;

export const isScopedStoragePath = (path: string, userId: string, docId: string): boolean =>
  path.startsWith(buildDocumentPathPrefix(userId, docId));
...
export const isPlaceholderImplementationEnabled = (
  ...
  const implementation = asNonEmptyString(env[CONVERTER_IMPLEMENTATION_ENV])?.toLowerCase();

  if (implementation === PLACEHOLDER_IMPLEMENTATION) return true;
  if (implementation === REAL_IMPLEMENTATION) return false;

  return normalizeEnabledFlag(asNonEmptyString(env[CONVERTER_PLACEHOLDER_ENABLED_ENV]));
};
...
export const validateConverterToken = (
  ...
  if (!providedToken || providedToken !== configuredToken) return "unauthorized";

  return "ok";
};

export const resolveConverterEndpointFromEnv = (env: NodeJS.ProcessEnv = process.env): string => {
