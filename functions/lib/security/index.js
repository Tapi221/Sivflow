...
exports.onSecurityLogCreated = void 0;
 ...
 * セキュリティログ生成時のトリガー
 */
exports.onSecurityLogCreated = functions.firestore
...
async function detectAbnormalPatterns(userId, logData) {
    const db = admin.firestore();
    const now = Date.now();
    let isAbnormal = false;
    for (const rule of DETECTION_RULES) {
        if (rule.type !== logData.eventType)
            continue;
        ...
        if (rule.timeWindowMs && rule.countThreshold) {
            ...
            const snapshot = await db.collection(`users/${userId}/securityLogs`)
            ...
            if (count >= rule.countThreshold) {
                console.warn(`[Abnormal Detect] User=${userId}, Type=${rule.type}, Count=${count}`);
                isAbnormal = true;
            }
        }
    }
...
async function calculateRiskScore(userId, logData) {
    const db = admin.firestore();
    ...
    return db.runTransaction(async (t) => {
        ...
        const matchedRule = DETECTION_RULES.find(r => r.type === logData.eventType);
        if (matchedRule) {
...
async function handleRiskActions(userId, deviceId, riskScore) {
    const db = admin.firestore();
    ...
    if (riskScore >= 21) {
        ...
        await db.collection(`users/${userId}/notifications`).add({
            type: 'SECURITY_ALERT',
            level: 'warning',
            message: '不審なアクセスが検知されました。アクティビティを確認してください。',
    ...
    if (riskScore >= 81) {
        ...
        const activeDevices = await devicesRef.where('status', '==', 'active').get();
        ...
        await batch.commit();
        console.log(`[Action] ALL DEVICES REVOKED and ACCOUNT LOCKED for user ${userId}`);
    }
}
