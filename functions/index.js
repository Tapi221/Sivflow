  ...
  try {
    await db.runTransaction(async (transaction) => {
      ...
      if (statsDoc.exists) {
        ...
        const newTotalCorrect = (currentData.totalCorrectCount || 0) + correctCount;
        const newTotalIncorrect = (currentData.totalIncorrectCount || 0) + incorrectCount;
        const newAccuracy = newTotalCorrect + newTotalIncorrect > 0
        ...
        transaction.update(statsRef, {
          ...
          totalIncorrectCount: admin.firestore.FieldValue.increment(incorrectCount),
    ...
    logger.info("Stats updated", { userId, date, correctCount, incorrectCount });
 ...
 * 学習ログ作成時に統計を自動更新
 */
exports.onStudyLogCreated = onDocumentCreated("studyLogs/{logId}", async (event) => {
  ...
  try {
    await db.runTransaction(async (transaction) => {
      ...
      if (statsDoc.exists) {
        const currentData = statsDoc.data();
        const newTotalCorrect = (currentData.totalCorrectCount || 0) + isCorrect;
        const newTotalIncorrect = (currentData.totalIncorrectCount || 0) + isIncorrect;
        const newAccuracy = newTotalCorrect + newTotalIncorrect > 0
          ? (newTotalCorrect / (newTotalCorrect + newTotalIncorrect)) * 100
          : 0;
        ...
        transaction.update(statsRef, {
          ...
          totalIncorrectCount: admin.firestore.FieldValue.increment(isIncorrect),
          accuracyRate: newAccuracy,
          lastStudyAt: now,
          updatedAt: now,
        });
      } else {
        const accuracy = isCorrect + isIncorrect > 0
...
exports.manualCleanupTrash = onRequest(async (req, res) => {
  ...
  try {
    const deletedItemsRef = db.collection("deletedItems");
    const expiredItems = await deletedItemsRef
      .where("deletedAt", "<", admin.firestore.Timestamp.fromDate(thresholdDate))
      .get();
    
    if (expiredItems.empty) {
      res.json({ success: true, deletedCount: 0, message: "No items to delete" });
      return;
    }
    ...
    res.json({ success: true, deletedCount: count, message: `Deleted ${count} items` });
  } catch (error) {
    ...
    switch (event.type) {
      case 'checkout.session.completed':
        ...
        if (userId) {
          ...
          logger.info('Subscription activated', { userId, subscriptionId: session.subscription });
        }
        break;
      ...
      case 'customer.subscription.updated':
        ...
        if (!updatedUserQuery.empty) {
          ...
          logger.info('Subscription updated', { userId: updatedUserId, status: updatedSubscription.status });
        }
        break;
    }
...
// Firebase Functions v2 の HTTPS onCall 関数を読み込む
