...
function normalizeAttributes(markup) {
  ...
  for (const [from, to] of SVG_ATTR_ALIASES) {
    normalized = normalized.replace(new RegExp(`\\b${escapeRegExp(from)}=`, 'g'), `${to}=`);
  }
  return normalized;
}
...
function normalizePaint(markup, prefersStroke) {
  let normalized = markup
    ...
    .replace(/\sfill="(?!none|currentColor|url\(#)([^"]+)"/g, ' fill="currentColor"');
...
function buildComponentSource(componentName, svgMarkup, usesIds) {
  const hookLine = usesIds ? "  const idPrefix = useId().replace(/:/g, '');\n\n" : '';
...
${hookLine}  return (
    ${svgMarkup.replace('className="block"', 'className={[\'block\', className].filter(Boolean).join(\' \')}').replace(
      '<svg ',
      '<svg ref={ref} {...props} '
    )}
  );
