  ...
  window.checkMigrationResult = async () => {
    ...
    try {
      ...
      if (queueCount >= 2) {
        ...
        log(
          `FAIL: syncQueue has ${queueCount} items (Expected >= 2).`,
          "error",
        );
        passed = false;
      }
      ...
      await new Promise((resolve) => {
        getAllReq.onsuccess = () => {
          ...
          if (migratedMeta) {
            log(
              `PASS: Found metadata with clientSeq >= 99991 (Current: ${migratedMeta.clientSeq}).`,
            );
          } else {
            log(
              `FAIL: No metadata found with expected clientSeq (>= 99991).`,
              "error",
            );
            console.table(metas);
            passed = false;
