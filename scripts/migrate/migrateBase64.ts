...
export async function detectAndMigrateBase64(
  userId: string,
    ...
    for (const card of cards) {
      ...
      if (card.questionImages) {
        for (let i = 0; i < card.questionImages.length; i++) {
          ...
          if (image.remoteUrl && isBase64DataUrl(image.remoteUrl as string)) {
            console.warn(
              `[Migration] Base64 detected in card ${card.id}, questionImages[${i}].remoteUrl`,
            );
          ...
          if (image.localUrl && isBase64DataUrl(image.localUrl as string)) {
            console.warn(
              `[Migration] Base64 detected in card ${card.id}, questionImages[${i}].localUrl`,
            );
      ...
      if (card.answerImages) {
        for (let i = 0; i < card.answerImages.length; i++) {
          ...
          if (image.remoteUrl && isBase64DataUrl(image.remoteUrl as string)) {
            console.warn(
              `[Migration] Base64 detected in card ${card.id}, answerImages[${i}].remoteUrl`,
            );
          ...
          if (image.localUrl && isBase64DataUrl(image.localUrl as string)) {
            console.warn(
              `[Migration] Base64 detected in card ${card.id}, answerImages[${i}].localUrl`,
            );
