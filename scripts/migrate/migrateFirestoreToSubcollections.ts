...
function initializeFirebase() {
  if (!fs.existsSync(SERVICE_ACCOUNT_PATH)) {
    console.error(`❌ サービスアカウントキーが見つかりません: ${SERVICE_ACCOUNT_PATH}`);
    console.error('   Firebase Console からサービスアカウントキーをダウンロードし、プロジェクトルートに配置してください。');
    process.exit(1);
  }
...
async function migrateFolders() {
  ...
  for (const doc of snapshot.docs) {
    ...
    // 新パスの参照
    const newFolderRef = db.collection('users').doc(userId).collection('folders').doc(docId);
    ...
    try {
      ...
      if (existingDoc.exists) {
        ...
        if (VERBOSE) {
          ...
          if (oldUpdatedAt && newUpdatedAt) {
            ...
            if (timeDiff !== 0) {
              console.log(`    差分: 旧=${oldUpdatedAt.toISOString()} / 新=${newUpdatedAt.toISOString()} (差: ${timeDiff}ms)`);
            }
          }
          console.log();
        }
      ...
      // 移行実行
      if (DRY_RUN) {
        console.log(`🔍 [DRY-RUN] フォルダ ${docId} → users/${userId}/folders/${docId}`);
...
async function migrateCards() {
  ...
  for (const doc of snapshot.docs) {
    ...
    // 新パスの参照
    const newCardRef = db.collection('users').doc(userId).collection('cards').doc(docId);
    ...
    try {
      ...
      // 移行実行
      if (DRY_RUN) {
        console.log(`🔍 [DRY-RUN] カード ${docId} → users/${userId}/cards/${docId}`);
...
function printStats() {
  ...
  console.log(`   スキップ:     ${stats.folders.skipped} 件 (既に新パスに存在)`);
  ...
  if (DRY_RUN) {
    console.log('ℹ️  DRY-RUN モードで実行しました。実際の書き込みは行われていません。');
  ...
  if (hasErrors) {
    console.warn('⚠️  一部のドキュメントでエラーが発生しました。ログを確認してください。\n');
  }

  if (hasNoUserId) {
    console.warn('⚠️  userId が存在しないドキュメントがありました。');
    console.warn('   これらのドキュメントは移行されていません。手動で確認してください。\n');
  }
}
...
async function main() {
  ...
  if (!DRY_RUN) {
    console.log('⚠️  警告: 本番モードで実行します。Firestoreに書き込みが行われます。');
    console.log('   10秒後に開始します。中止する場合は Ctrl+C を押してください。\n');
